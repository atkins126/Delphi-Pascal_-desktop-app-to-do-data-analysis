...
////      procedure Progress(Sender: TObject; Stage: TProgressStage; PercentDone: Byte;  RedrawNow: Boolean; const R: TRect; const Msg: AnsiString; var Continue: Boolean); Virtual;
    public
...
{      procedure Assign(Source: TPersistent); override;
      ...
      // Saving and loading
      procedure LoadFromStream (Str:TStream; Handler:TFPCustomImageReader); overload;
      procedure LoadFromStream (Str:TStream); overload;
      procedure LoadFromFile (const filename:String; Handler:TFPCustomImageReader); overload;
      ...
      procedure SaveToFile (const filename:String; Handler:TFPCustomImageWriter); overload;
...
////      FOnProgress : TFPImgProgressEvent;
    ...
    protected
////      procedure Progress(Stage: TProgressStage; PercentDone: Byte;  RedrawNow: Boolean; const R: TRect; const Msg: AnsiString; var Continue: Boolean); Virtual;
...
begin
  ...
  if LowerCase(S) = 'white' then AName := hcnWhite else
  if LowerCase(S) = 'silver' then  AName := hcnSilver else
  if LowerCase(S) = 'gray' then    AName := hcnGray else
  if LowerCase(S) = 'black' then   AName := hcnBlack else
  if LowerCase(S) = 'red' then     AName := hcnRed else
  if LowerCase(S) = 'maroon' then  AName := hcnMaroon else
  if LowerCase(S) = 'yellow' then  AName := hcnYellow else
  if LowerCase(S) = 'olive' then   AName := hcnOlive else
  if LowerCase(S) = 'lime' then    AName := hcnLime else
  if LowerCase(S) = 'green' then   AName := hcnGreen else
  if LowerCase(S) = 'aqua' then    AName := hcnAqua else
  if LowerCase(S) = 'teal' then    AName := hcnTeal else
  if LowerCase(S) = 'blue' then    AName := hcnBlue else
  if LowerCase(S) = 'navy' then    AName := hcnNavy else
  if LowerCase(S) = 'fuchsia' then  AName := hcnFuchsia else
  if LowerCase(S) = 'purple' then AName := hcnPurple else
    Result := False;
end;
