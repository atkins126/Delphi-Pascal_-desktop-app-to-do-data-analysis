...
uses
  ...
  System.Rtti, System.DateUtils, System.Math,
  uItemanExport, FMX.Forms, FMX.Objects, FMX.Graphics, FMX.Types, StrUtils;
...
//    FItem: Integer;
    ...
    FFlagArray: array[1..8] of string; //columns 6&7 now LM HM LowMean HighMean, column 8 is DIF
    ...
    procedure mantelH(item : integer; responsearray, iteminfoarray : TChar2Array; difrankarray : TIntegerArray; difarray : TRealArray);
    ...
    procedure ScoreGroups(RankingArray, ReduceArray, scorearray : TIntegerArray; iteminfoarray, responsearray : TChar2Array );
    procedure SumStats(iteminfoarray : TChar2Array; scorearray : TIntegerArray);
    procedure OptionStats(iteminfoarray, responsearray : TChar2Array; itemscalearray,nkeyarray,scorearray : TIntegerArray);
    function cdfni(p: single): Single;
    procedure ScaleStats(iteminfoarray: TChar2Array);
    procedure EstimateKR20Without(ResponseArray, ItemInfoArray : TChar2Array; itemscalearray,nkeyarray,scorearray : TIntegerArray);
    procedure DomainStats(iteminfoarray, responsearray : TChar2Array; itemscalearray : TIntegerArray; domscalearray : TRealArray; nkeyarray : TIntegerArray);
    ...
    procedure Reliability(splitscore1, splitscore2, splitdomain1, splitdomain2, itemscalearray : TIntegerArray; iteminfoarray, responsearray : TChar2Array);
    ...
    procedure SubGroups(aItemInfoArray, aResponseArray : TChar2Array; aOmch : char; aI: Integer; aRankIngArray, aReduceArray, aItemsCaleArray : TIntegerArray);
    ...
    procedure PrintItemTables(const prec : string; item : integer;
          iteminfoarray : TChar2Array; itemidarray : TStringArray;
          itemscalearray, nkeyarray : TIntegerArray;
          start0 : char; difarray : TRealArray);
    function Livingston: real;
    procedure DrawDistGraph(Min, Max : real; nit, nunit, call: integer; graph: TBitmap; title : string);
    procedure DrawScatterplot(Min, Max : real; nit, nunit, call: integer; graph : TBitmap;
    ...
    procedure DrawItemGraph(graph : TBitmap; item : integer; ItemScaleArray : TIntegerArray;
...
begin
  ...
  i := 0;
  while (i <= Length(AFlags)) or (i <= Length(FFlagArray)) do
  begin
...
function TItemanCalculation.RunIteman(): Boolean;
var
  ...
  begin
    ...
    begin
      ...
      if not usemrf then if frmMain.eOutputFile.Text = '' then
      begin
    ...
    try
      if frmMain.omitcharbox.Text[1] = chr(9) then
        frmMain.omitcharbox.Text:= chr(32);
    except
      ErrorMessage:= ('The omit character box is blank. Please specify an omit character code and retry the analysis.');
      FAllOk:= False;
      lInputOk:= False;
      Result:= False;
      Exit;
    ...
    try
      if frmMain.nacharbox.Text[1] = chr(9) then
        frmMain.nacharbox.Text:= chr(32);
    except
      ErrorMessage:=('The not administered character box is blank. Please specify a not administered character code and retry the analysis.');
      FAllOk:= False;
      lInputOk:= False;
      Result:= False;
      Exit;
    ...
    if (frmMain.omitcharbox.Text[1] = frmMain.nacharbox.Text[1]) then
    begin
      ErrorMessage:=('The omit character and not administered character are the same.' + #13 + 'Please use different characters and retry the analysis.');
      FAllOk:= False;
      lInputOk:= False;
      Result:= False;
      Exit;
    ...
    if not (frmMain.cbDataMatrixFile.IsChecked) then
      if (frmMain.itemColumnBox.Text = '0') then
      begin
        ErrorMessage:= ('The "Item responses begin in column" box is set to 0' +#13 + 'Check the data file and specify a number greater than 0');
        FAllOk:= False;
        lColumnOk:= False;
        Result:= False;
        Exit;
      end else
      if (frmMain.itemColumnBox.Text <> '0') and
         ((StrToIntDef(frmMain.eTab2Numbers1.Text, 0) + StrToIntDef(frmMain.IDColumnsBegin.Text, 0)) > StrToIntDef(frmMain.itemColumnBox.Text, 0)) then
      begin
        ErrorMessage:= ('There are ' + frmMain.eTab2Numbers1.Text + ' columns of examinee ID that begin in column ' +
        frmMain.IDColumnsBegin.Text + #13
         + 'The "item responses begin in column" box should be at least ' +
  ...
  begin
    ...
    try
      ...
      lInputOk:= False;
      ErrorMessage:= ('The data matrix file is already open in another program and cannot be opened by Iteman.' + #13 + 'Please close the file, then hit Run againe to restart the analysis.');
      Result:= False;
    ...
    begin //checking to see if the data matrix has blank lines at the beginning
      ...
      begin
        lInputOk:= False;
        ErrorMessage:=('The data matrix file has blank line(s) at the beginning.'+#13 + 'Please delete the blank line(s) and try again.');
        Result:= False;
        CloseFile(lDataMatrixFile);
        Exit;
      end;
      ...
      begin
        lInputOk:= False;
        ErrorMessage:= ('The data matrix file is a MS WORD document.' + #13 + 'Please open the document in a word processor and save as a Plain Text file.');
      ...
      begin
        ...
        begin
          lInputOk:= False;
          ErrorMessage:=('The data matrix file is not an ANSI text file.' + #13 + 'Please open the document in a word processor and save as a Plain Text file.');
          CloseFile(lDataMatrixFile);
          Exit;
        end;
      end;
      ...
      begin
        ErrorMessage:= ('The data matrix file did not contain any tabs on the first line although the file was ' + #13 + 'identified as tab delimited input. We proceed with the analysis without delimited input.');
      ...
      begin
        ErrorMessage:= ('The data matrix file did not contain any commas on the first line although the file was ' + #13 + 'identified as comma delimited input. We proceed with the analysis without delimited input.');
        frmMain.delimitbox.IsChecked:= False;
      ...
      begin
        ErrorMessage:= ('The data matrix file contained both tabs and commas for delimiters.' + #13 + 'Proceed with the analysis using just tabs for the delimiters?' + #13 + #13 + 'Note: Only click Yes if the commas were included as part of the examinee ID.');
      ...
      begin
        ErrorMessage:=('The data matrix file contains tabs.' + #13 + 'We proceed with the analysis using tab delimited input');
        frmMain.delimitbox.IsChecked:= True;
        frmMain.tabbox.IsChecked:= True;
        lInputOk:= True;
      ...
      begin
        ErrorMessage:= ('The data matrix file contains commas.' + #13 + 'We change the program settings to use comma delimited input.');
        frmMain.delimitbox.IsChecked:= True;
      ...
      begin
        ...
        try
          ...
          lInputOk:= False;
          ErrorMessage:=('The item control file is already open in another program and cannot be opened by Iteman.' + #13 + 'Please close the file, then hit Run again to restart the analysis.');
          Result:= False;
          Exit;
        end;
    ...
    begin
      ...
      begin
        ...
        if lChar = chr(9) then lInputOk:= True;
        if lChar = chr(9) then controldelim:= chr(9);
      end;
      ...
      begin
        ErrorMessage:= ('The item control file is invalid.' + #13 + 'The item control file must be a tab or comma delimited plain text file.');
        Result:= False;
        Exit;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ErrorMessage:= ('The number of items in your Item Control File and the first line of your data file mismatch.' + #13 + 'Please correct this error and re-run your data. This run is terminated.');
        lInputOk:= False;
        Result:= False;
        Exit;
       end;
    ...
    begin
      ...
      begin
        ReadLn(lItemInfoFile,lChar);   //chr(13) is return, so blank lines are not treated as items
      ...
      begin
        ...
        for lColumn:= 1 to pred(StrToIntDef(frmMain.itemColumnBox.Text, 0)) do
          Read(lDataMatrixFile,lChar); // parse everyout out before response
      ...
      if (lNumberOfEntries <> FNumberOfItems) then
      begin
        ErrorMessage:=('The number of items in your Item Control File and the first line of your data file mismatch.' + #13 + 'Please correct this error and re-run your data.  This run is terminated.');
        lInputOk:= False;
        Result:= False;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        lInputOk:= False;
        ErrorMessage:=('The Main CSV output file is already open in another program and cannot be saved by Iteman.' + #13 + 'Please close the file, then hit Run to restart the analysis.');
        Result:= False;
        Exit;
      end;
      ...
      try
        ...
        lInputOk:= False;
        ErrorMessage:= ('The Scores CSV file is already open in another program and cannot be saved by Iteman.' + #13 + 'Please close the file, then hit Run to restart the analysis.');
        Result:= False;
        Exit;
      end;
      ...
      begin
        ...
        try
          ...
          lInputOk := False;
          ErrorMessage:=('The scored data matrix file is already open in another program and cannot be saved by Iteman.' + #13 + 'Please close the file, then hit Retry to continue the analysis.');
          Result:= False;
          Exit;
        end;
      end;
      ...
      begin
        ...
        try
          ...
          lInputOk:= False;
          ErrorMessage:=('The external score file is already open in another program and cannot be opened by Iteman.' + #13 + 'Please close the file, then hit Retry to continue the analysis.');
          Result:= False;
          Exit;
        end;
      end;
  ...
  begin
    ...
    if (StrToIntDef(frmMain.Precision.Text, 0) = 2) then prec:= '0.00';
    if (StrToIntDef(frmMain.Precision.Text, 0) = 3) then prec:= '0.000';
    if (StrToIntDef(frmMain.Precision.Text, 0) = 4) then prec:= '0.0000';

    {Write examinee results to scores csv file}
    if not ScoredRate then
       Write(lScoresFile, 'Sequence,ID,All Items,Scored Items,Pretest items,Scored Proportion Correct,Rank,Percentile,Group')
    else
       Write(lScoresFile, 'Sequence,ID,All Items,Scored Items,Pretest items,Rank,Percentile,Group');

    if frmMain.cbExternalScoreFile.IsChecked then Write(lScoresFile, ',External');
    if FNumDomains > 1 then For lDomain:= 1 to FNumDomains do write(lScoresFile, ',', FStoreDomain[lDomain]);
    if frmMain.ScaledCheckBox.IsChecked then Write(lScoresFile, ',Scaled Total Score');
    ...
    if frmMain.classcheckbox.IsChecked then write(lScoresFile, ',Classification');
    if not ScoredRate then write(lScoresFile,',CSEM III,CSEM IV');
    ...
    begin
      Write(lScoresFile, lRow, ',', lExamineeIDArray[lRow], ',', lScoreArray[lRow,4], ',', lScoreArray[lRow,1], ',', lScoreArray[lRow,5]);
...
//          FProppassing:= FProppassing + 1;
    ...
    if frmMain.ScaledCheckBox.IsChecked then Write(lOutputFile, ',Scaled');
    if frmMain.cbExternalScoreFile.IsChecked then Write(lOutputFile, ',External');
    if FNumDomains > 1 then
      for lDomain:= 1 to FNumDomains do
    ...
    if FPreTest > 0 then
      write(lOutputFile, 'Items:,', FTestStatsArray[1,1]:3+pr:pr, ',', FTestStatsArray[2,1]:3+pr:pr, ',', FTestStatsArray[3,1]:3+pr:pr)
    else
      write(lOutputFile, 'Items:,', FTestStatsArray[1,1]:3+pr:pr);

    if frmMain.ScaledCheckBox.IsChecked then Write(lOutputFile, ',', FTestStatsArray[4,1]:3+pr:pr);
    if frmMain.cbExternalScoreFile.IsChecked then Write(lOutputFile, ',', FTestStatsArray[5,1]:3+pr:pr);
    if FNumDomains > 1 then
      for lDomain:= 1 to FNumDomains do
    ...
    if FPreTest > 0 then
      write(lOutputFile, 'Mean:,', FTestStatsArray[1,2]:3+pr:pr, ',', FTestStatsArray[2,2]:3+pr:pr, ',', FTestStatsArray[3,2]:3+pr:pr)
    else
      write(lOutputFile, 'Mean:,', FTestStatsArray[1,2]:3+pr:pr);

    if frmMain.ScaledCheckBox.IsChecked then Write(lOutputFile, ',', FTestStatsArray[4,2]:3+pr:pr);
    if frmMain.cbExternalScoreFile.IsChecked then Write(lOutputFile, ',', FTestStatsArray[5,2]:3+pr:pr);
    if FNumDomains > 1 then
      for lDomain:= 1 to FNumDomains do
    ...
    if FPreTest > 0 then
      write(lOutputFile, 'SD:,', FTestStatsArray[1,3]:3+pr:pr, ',', FTestStatsArray[2,3]:3+pr:pr, ',', FTestStatsArray[3,3]:3+pr:pr)
    else
      write(lOutputFile, 'SD:,', FTestStatsArray[1,3]:3+pr:pr);

    if frmMain.ScaledCheckBox.IsChecked then Write(lOutputFile, ',', FTestStatsArray[4,3]:3+pr:pr);
    if frmMain.cbExternalScoreFile.IsChecked then Write(lOutputFile, ',', FTestStatsArray[5,3]:3+pr:pr);
    if FNumDomains > 1 then
      for lDomain:= 1 to FNumDomains do
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'Min Score:,', FTestStatsArray[1,4]:3+pr:pr, ',', FTestStatsArray[2,4]:3+pr:pr, ',', FTestStatsArray[3,4]:3+pr:pr)
    else
      write(lOutputFile,  'Min Score:,', FTestStatsArray[1,4]:3+pr:pr);

    if frmMain.ScaledCheckBox.IsChecked then Write(lOutputFile, ',', FTestStatsArray[4,4]:3+pr:pr);
    if frmMain.cbExternalScoreFile.IsChecked then Write(lOutputFile, ',', FTestStatsArray[5,4]:3+pr:pr);
    if FNumDomains > 1 then
      for lDomain:= 1 to FNumDomains do
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'Max Score:,', FTestStatsArray[1,5]:3+pr:pr, ',', FTestStatsArray[2,5]:3+pr:pr, ',', FTestStatsArray[3,5]:3+pr:pr)
    else
      write(lOutputFile,  'Max Score:,', FTestStatsArray[1,5]:3+pr:pr);

    if frmMain.ScaledCheckBox.IsChecked then Write(lOutputFile, ',', FTestStatsArray[4,5]:3+pr:pr);
    if frmMain.cbExternalScoreFile.IsChecked then Write(lOutputFile, ',', FTestStatsArray[5,5]:3+pr:pr);
    ...
    begin
      if FPreTest > 0 then
        write(lOutputFile, 'Mean P:,', FTestStatsArray[1,7]:3+pr:pr, ',', FTestStatsArray[2,7]:3+pr:pr, ',', FTestStatsArray[3,7]:3+pr:pr)
      else
        write(lOutputFile,  'Mean P:,', FTestStatsArray[1,7]:3+pr:pr);

      if frmMain.ScaledCheckBox.IsChecked then Write(lOutputFile, ',-');
      if frmMain.cbExternalScoreFile.IsChecked then Write(lOutputFile, ',-');
      if FNumDomains > 1 then
        for lDomain:= 1 to FNumDomains do
    ...
    begin
      if FPreTest > 0 then
        write(lOutputFile,  'Item Mean:,', FTestStatsArray[1,16]:3+pr:pr, ',', FTestStatsArray[2,16]:3+pr:pr, ',', FTestStatsArray[3,16]:3+pr:pr)
      else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'Mean R:,', FTestStatsArray[1,8]:3+pr:pr, ',', FTestStatsArray[2,8]:3+pr:pr, ',', FTestStatsArray[3,8]:3+pr:pr)
    else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'Alpha:,', FTestStatsArray[1,6]:3+pr:pr, ',', FTestStatsArray[2,6]:3+pr:pr, ',', FTestStatsArray[3,6]:3+pr:pr)
    else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'SEM:,', FTestStatsArray[1,9]:3+pr:pr, ',', FTestStatsArray[2,9]:3+pr:pr, ',',FTestStatsArray[3,9]:3+pr:pr)
    else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'Split-Half (Random):,', FTestStatsArray[1,10]:3+pr:pr, ',', FTestStatsArray[2,10]:3+pr:pr, ',','-')
    else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'Split-Half (First-Last):,', FTestStatsArray[1,11]:3+pr:pr, ',', FTestStatsArray[2,11]:3+pr:pr, ',','-')
    else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'Split-Half (Odd-Even):,', FTestStatsArray[1,12]:3+pr:pr, ',', FTestStatsArray[2,12]:3+pr:pr, ',','-')
    else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'S-B Random:,', FTestStatsArray[1,13]:3+pr:pr, ',', FTestStatsArray[2,13]:3+pr:pr, ',','-')
    else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'S-B First-Last:,', FTestStatsArray[1,14]:3+pr:pr, ',', FTestStatsArray[2,14]:3+pr:pr, ',', '-')
    else
    ...
    if FPreTest > 0 then
      write(lOutputFile,  'S-B Odd-Even:,', FTestStatsArray[1,15]:3+pr:pr, ',', FTestStatsArray[2,15]:3+pr:pr, ',','-')
    else
    ...
    if FIsMult and not FIsRate then
      if FNumDomains > 1 then
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,P,Total Rpbis,Total Rbis,Domain Rpbis,Domain Rbis,Alpha w/o,Flags,Omit Freq,NR Freq,')
      else
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,P,Total Rpbis,Total Rbis,Alpha w/o,Flags,Omit Freq,NR Freq,');

    if FIsMult and FIsRate then
      if FNumDomains > 1 then
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,Mean P/Item Mean,Total Rpbis/R,Total Rbis/R,Domain Rpbis/R,Domain Rbis/R,Alpha w/o,Flags,Omit Freq,NR Freq,')
      else
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,Mean P/Item Mean,Total Rpbis/R,Total Rbis/R,Alpha w/o,Flags,Omit Freq,NR Freq,');

    if not FIsMult and FIsRate then
      if FNumDomains > 1 then
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,Item Mean,Total R,Total Eta,Domain R,Domain Eta,Alpha w/o,Flags,Omit Freq,NR Freq,')
      else
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,Item Mean,Total R,Total Eta,Alpha w/o,Flags,Omit Freq,NR Freq,');

    if lAllPC then pcint:= 1 else pcint:= 0;
    ...
    for lRow := 1 to FNumberOfItems do
      ...
      begin
        ...
        if lNKeyArray[lRow,1] = 1 then
          Write(lOutputFile, lRow, ',', lItemIDArray[lRow], ',', lItemInfoArray[lRow,1], ',', PreText, ',', lItemScaleArray[lRow,1], ',', FStoreDomain[lItemScaleArray[lRow,2]], ',', FItemStatsArray[lRow,1]:1+pr:pr, ',', FItemStatsArray[lRow,2]:1+pr:pr, ',', FItemStatsArray[lRow,3]:1+pr:pr, ',', FItemStatsArray[lRow,4]:1+pr:pr, ',');
        ...
        begin
          ...
          write(lOutputFile, ',', PreText, ',', lItemScaleArray[lRow,1], ',', FStoreDomain[lItemScaleArray[lRow,2]], ',', FItemStatsArray[lRow,1]:1+pr:pr, ',', FItemStatsArray[lRow,2]:1+pr:pr, ',', FItemStatsArray[lRow,3]:1+pr:pr, ',', FItemStatsArray[lRow,4]:1+pr:pr, ',');
        end;
        ...
        begin
          if FItemStatsArray[lRow,2] < FMinP then write(lOutputFile, FFlagArray[2], ' ');
          if FItemStatsArray[lRow,2] < FMinP then lItemIsFlagged := True;
          if FItemStatsArray[lRow,2] > FMaxP then write(lOutputFile, FFlagArray[3], ' ');
          if FItemStatsArray[lRow,2] > FMaxP then lItemIsFlagged := True;
        end;

        if lItemInfoArray[lRow,3] <> 'M' then
        begin
          if FItemStatsArray[lRow,2] < FMinMean then write(lOutputFile, FFlagArray[6], ' ');
          if FItemStatsArray[lRow,2] < FMinMean then lItemIsFlagged := True;
          if FItemStatsArray[lRow,2] > FMaxMean then write(lOutputFile, FFlagArray[7], ' ');
          if FItemStatsArray[lRow,2] > FMaxMean then lItemIsFlagged := True;
        end;

        if FItemStatsArray[lRow,3] < FMinR then write(lOutputFile, FFlagArray[4], ' ');
        if FItemStatsArray[lRow,3] < FMinR then lItemIsFlagged := True;
        if FItemStatsArray[lRow,3] > FMaxR then write(lOutputFile, FFlagArray[5], ' ');
        if FItemStatsArray[lRow,3] > FMaxR then lItemIsFlagged := True;

        if (lItemInfoArray[lRow,9] = 'D') and (frmMain.difcheckbox.IsChecked) then
        begin
          if lDifarray[lRow,5] < 0.05 then lItemIsFlagged := True;
          if lDifarray[lRow,5] < 0.05 then write(lOutputFile, FFlagArray[8], ' ');
        end; //DIF coding

        if lItemIsFlagged then inc(FNumFlagItems);
        if lItemIsFlagged then FFlagID[lRow] := lRow;
    ...
    begin
      ...
      for lRow := 1 to FNumberOfItems do if (lItemInfoArray[lRow,2] = 'Y') and (lItemInfoArray[lRow,9] = 'D') then
      begin
        Write(lOutputFile, lRow, ',', lItemIDArray[lRow], ',');
        write(lOutputFile, lDifarray[lRow,1]:1+pr:pr, ',',
        lDifarray[lRow,3]:1+pr:pr, ',', lDifarray[lRow,4]:1+pr:pr, ',', lDifarray[lRow,5]:1+pr:pr);
        if (lDifarray[lRow,5] < 0.05) and (lDifarray[lRow,4] < 0) then write(lOutputFile,',',frmMain.Group2label.Text);
        if (lDifarray[lRow,5] < 0.05) and (lDifarray[lRow,4] > 0) then write(lOutputFile,',',frmMain.Group1label.Text);
        if lDifarray[lRow,5] > 0.05 then write(lOutputFile,',');
        for lColumn := 1 to StrToIntDef(frmMain.NumDifGroups.Text, 0) do
    ...
    begin
      if FNumDomains > 1 then
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,P,Total Rpbis,Total Rbis,Domain Rpbis,', 'Domain Rbis,Alpha w/o,Flags,Option,Option N,Option Prop,Option Rpbis,Option Rbis,Option Mean,Option SD,')
      else
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,P,Total Rpbis,Total Rbis,Alpha w/o,Flags,', 'Option,Option N,Option Prop,Option Rpbis,Option Rbis,Option Mean,Option SD,');
    end; //if mult and no rate

    if FIsMult and FIsRate then
    begin
      if FNumDomains > 1 then
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,Item Mean/P,Total Rpbis/R,Total Rbis/Eta,Domain Rpbis/R,', 'Domain Rbis/Eta,Alpha w/o,Flags,Option,Option N,Option Prop,Option Rpbis,Option Rbis,Option Mean,Option SD,')
      else
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,Item Mean/P,Total Rpbis/R,Total Rbis/Eta,Alpha w/o,Flags,', 'Option,Option N,Option Prop,Option Rpbis,Option Rbis,Option Mean,Option SD,');
    end; //if mult and rate

    if not FIsMult and FIsRate then
    begin
      if FNumDomains > 1 then
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,Item Mean,Total R,Total Eta,Domain R,', 'Domain Eta,Alpha w/o,Flags,Option,Option N,Option Prop,Option Rpbis,Option Rbis,Option Mean,Option SD,')
      else
        Write(lOutputFile, 'Sequence,Item ID,Key,Scored,NumOptions,Domain,N,Item Mean,Total R,Total Eta,Alpha w/o,Flags,', 'Option,Option N,Option Prop,Option Rpbis,Option Rbis,Option Mean,Option SD,');
    ...
    for lRow := 1 to FNumberOfItems do
      ...
      begin
        {This is columns pertaining to FItem info}
        if lItemInfoArray[lRow,2] = 'P' then PreText := 'Pretest' else PreText := 'Yes';
        if lItemInfoArray[lRow,2] = 'Y' then
          PreStr:= formatfloat('0.000',FItemStatsArray[lRow,5])
        else
          PreStr:= 'NA';
        ...
        if lCheckKey then write(lOutputFile,FFlagArray[1], ' ');
        ...
        SubGroups(lItemInfoArray,lResponseArray,FOmch,lRow,lRankingArray,lReduceArray,lItemScaleArray);   //added 4/12/12

        {output first lRow of option stats}
        if (FRespAsInt = True) and (lItemInfoArray[lRow,3] <> 'P') then write(lOutputFile,',1,');
        if lItemInfoArray[lRow,3] = 'P' then write(lOutputFile,',0,');
        if (FRespAsInt = False) and (lItemInfoArray[lRow,3] <> 'P') then write(lOutputFile,',',chr(65),',');
        ...
        if ConvertResponseToInteger(lItemInfoArray[lRow,1],lItemInfoArray,lRow,lItemScaleArray[lRow,1]) = 1-SCO then
        ...
        begin
          ...
          if FNumDomains > 1 then write(lOutputFile,',,');
          if lColumn <= lItemScaleArray[lRow,1] then
          begin
            if (FRespAsInt = True) and (lItemInfoArray[lRow,3] <> 'P') then write(lOutputFile,',,',lColumn);
            if lItemInfoArray[lRow,3] = 'P' then write(lOutputFile,',,',lColumn-1);
            if (FRespAsInt = False) and (lItemInfoArray[lRow,3] <> 'P') then write(lOutputFile,',,',chr(lColumn+64));
          end;
          ...
          begin
            ...
            if not frmMain.OmitAsinc.IsChecked then
              write(lOutputFile, ',', FOptionPropsArray[lRow,16]:1+pr:pr) else write(lOutputFile,',-');
          ...
          begin
            ...
            begin
              ...
              Write(lOutputFile, ',', FOptionMeansArray[lRow,lColumn]:1+pr:pr);
              Write(lOutputFile, ',', FOptionSDArray[lRow,lColumn]:1+pr:pr);
              for jfor := 1 to FNumCut do Write(lOutputFile, ',', FSubGroupPArray[jfor+(lColumn-1)*FNumCut]:1+pr:pr);
            end else
              Write(lOutputFile, ',-,-,-,-,-,-');

            if ConvertResponseToInteger(lItemInfoArray[lRow,1],lItemInfoArray,lRow,lItemScaleArray[lRow,1]) = lColumn-SCO then
              Write(lOutputFile, ',**KEY**');

            jfor:= 1;
            if lNKeyArray[lRow,1] > 1 then
              if ConvertResponseToInteger(lItemInfoArray[lRow,3+jfor],lItemInfoArray,lRow,lItemScaleArray[lRow,1]) = lColumn-SCO then
              begin
...
//                inc(jfor); // Never used
  ...
  begin
    ...
    begin
      ...
      try
        ...
        lInputOk := False;
        ErrorMessage:=('The BBO-matrix CSV file is already open in another program and cannot be saved by Iteman.'+#13 + 'Please close the file, then hit Retry to continue the analysis.');
        Result:= False;
        Exit;
      end;
      ...
      try
        ...
        lInputOk:= False;
        ErrorMessage:=('The EIC-matrix CSV file is already open in another program and cannot be saved by Iteman.'+#13 + 'Please close the file, then hit Retry to continue the analysis.');
        Result:= False;
        Exit;
      end;
      ...
      try
        ...
        lInputOk:= False;
        ErrorMessage:=('The EEIC-matrix CSV file is already open in another program and cannot be saved by Iteman.'+#13 + 'Please close the file, then hit Retry to continue the analysis.');
        Result:= False;
        Exit;
      end; //lInputOk
  ...
  var
    ...
    function calcbin (max : integer) : integer;
     var ij, temp1 : integer;
    begin
      nbin := 0;
     for ij := 9 to 20 do begin
       temp1 := ((brange) mod ij);
       if temp1 = 0 then nbin := ij+1;
     end;
       if nbin = 0 then nbin := 20+1;     //if 0 then no even divisor...
    end;
  begin
    ...
    try
      ...
      AddSpecifications(FSpecBitmap, frmMain.GetInputFiles(data, ctrl), frmMain.GetOutputFiles(data, ctrl), GetTableSpec, usemrf);
      ...
      AddText('');
      AddText(PWideChar('Figure 1 displays the distribution of the raw scores for the scored items across all domains. ' +
      'Table ' + inttostr(tableno) + ' displays the frequency distribution for total score shown in Figure 1.'));
      ...
      DrawDistGraph(FTestStatsArray[1,4],FTestStatsArray[1,5],round(FTestStatsArray[1,1]),FNumberOfExaminees,1, FGraphBitmap,'Total Score');
      ...
      begin
        ...
        AddText('');
        AddText(PWideChar('Figure ' + inttostr(figureno) + ' displays the distribution of the raw scores for '+FStoreDomain[lRow]+'. '+
          'Table '+inttostr(tableno)+' displays the frequency distribution of domain scores shown in Figure '+inttostr(figureno)+'.'));
        {Write description of Figure 1 to docx outpout}
        AddText('');
        AddText('');
        ...
        if FDomainStatsArray[lRow,5] >= 20 then calcbin(round(FDomainStatsArray[lRow,5]));

        CalcGF(FDomainStatsArray[lRow,4],FDomainStatsArray[lRow,5],lRow+1,lScoreArray);
        DrawDistGraph(FDomainStatsArray[lRow,4],FDomainStatsArray[lRow,5],round(FDomainStatsArray[lRow,1]),FNumberOfExaminees,1, FGraphBitmap, FStoreDomain[lRow]);
      ...
      begin
        figureno := figureno + 1;
        //FExport.AddText('');
        AddText(PWideChar('Figure ' + inttostr(figureno) + ' displays the distribution of the P values for the dichotomously scored items (correct/incorrect). '+
                        'Table '+inttostr(tableno)+' displays the frequency distribution of the P values shown in Figure '+inttostr(figureno)+'.'));
        AddText('');
        nbin := 10;
        brange := 1;
      ...
      begin
        ...
        AddText(PWideChar('Figure ' + inttostr(figureno) + ' displays the distribution of the Item Means '+
                        '(rating scale or partial credit items only).  Table '+inttostr(tableno)+' displays the frequency distribution of the Item Means shown in Figure '+inttostr(figureno)+'.'));
        AddText('');
        nbin := 10;
        if IsZero = True then i := 0 else i := 1;
        if IsZero = True then j := 1 else j := 2;
        if maxcategory > 2 then FMinItem1 := floor(FMinItem1);
        if maxcategory > 2 then FMaxItem1 := ceil(FMaxItem1);
        if (FMaxItem1 - FMinItem1) < 1 then brange := 1 else brange := round(FMaxItem1 - FMinItem1);
        if maxcategory = 2 then FMinItem1 := i;
        if maxcategory = 2 then FMaxItem1 := j;
      ...
      begin
        ...
        figureno := figureno + 1;
        AddText('');
        AddText(PWideChar('Figure ' + inttostr(figureno) + ' displays the distribution of the Point-Biserial Correlations '+
          'for the dichotomously scored items (correct/incorrect).  Table '+inttostr(tableno)+' displays the frequency distribution of the Point-Biserial correlations shown in Figure '+inttostr(figureno)+'.'));
        AddText('');
        if (FMinRPBis < 0) and (FMinRPBis > -0.1) then Rrange := 1.1;
        if (FMinRPBis < -0.1) and (FMinRPBis > -0.2) then Rrange := 1.2;
        if (FMinRPBis < -0.2) and (FMinRPBis > -0.3) then Rrange := 1.3;
        if (FMinRPBis < -0.3) and (FMinRPBis > -0.4) then Rrange := 1.4;
        if (FMinRPBis < -0.4) and (FMinRPBis > -0.5) then Rrange := 1.5;
        if (FMinRPBis < -0.5) and (FMinRPBis > -0.6) then Rrange := 1.6;
        if (FMinRPBis < -0.6) and (FMinRPBis > -0.7) then Rrange := 1.7;
        if (FMinRPBis < -0.7) and (FMinRPBis > -0.8) then Rrange := 1.8;
        if (FMinRPBis < -0.8) and (FMinRPBis > -0.9) then Rrange := 1.9;
        if (FMinRPBis < -0.9) and (FMinRPBis > -1.0) then Rrange := 2.0;
      ...
      begin
        ...
        figureno := figureno + 1;
        AddText('');
        AddText(PWideChar('Figure ' + inttostr(figureno) + ' displays the distribution of the Item-Total Pearson r Correlations '+
          'for polytomously scored items (rating scale or partial credit). Table '+inttostr(tableno)+' displays the frequency distribution of Pearsons r correlations shown in Figure '+inttostr(figureno)+'.'));
        AddText('');
        if (FMinCorr < 0) and (FMinCorr > -0.1) then Rrange := 1.1;
        if (FMinCorr < -0.1) and (FMinCorr > -0.2) then Rrange := 1.2;
        if (FMinCorr < -0.2) and (FMinCorr > -0.3) then Rrange := 1.3;
        if (FMinCorr < -0.3) and (FMinCorr > -0.4) then Rrange := 1.4;
        if (FMinCorr < -0.4) and (FMinCorr > -0.5) then Rrange := 1.5;
        if (FMinCorr < -0.5) and (FMinCorr > -0.6) then Rrange := 1.6;
        if (FMinCorr < -0.6) and (FMinCorr > -0.7) then Rrange := 1.7;
        if (FMinCorr < -0.7) and (FMinCorr > -0.8) then Rrange := 1.8;
        if (FMinCorr < -0.8) and (FMinCorr > -0.9) then Rrange := 1.9;
        if (FMinCorr < -0.9) and (FMinCorr > -1.0) then Rrange := 2.0;
        nbin := round(Rrange*10);
        brange := 1;
      ...
      begin
        ...
        if lScoredMult = True then
          AddText('');
        AddText(PWideChar('Figure ' + inttostr(figureno) + ' displays the scatterplot of the Item Means (difficulty) by R '+
          '(discrimination) for polytomously scored items (rating scale or partial credit).'));
        if IsZero = True then brange := 0 else brange := 1;
        if lAllPC = True then
          DrawScatterplot(brange,maxcategory-1,FNumRate[1],FNumberOfItems,2,FGraphBitmap,lItemInfoArray,'R','Item Mean')
        else
          DrawScatterplot(brange,maxcategory,FNumRate[1],FNumberOfItems,2,FGraphBitmap,lItemInfoArray,'R','Item Mean'); //lAllPC is all partial credit (begins at 0)
        ;
      ...
      begin
        AddText('');
        tmp := 'Figure '+inttostr(figureno+1)+' displays a graph of the Conditional Standard Error of Measurement (CSEM) Formula IV. ';
        if (rawcut >= 0) and (rawcut <= FTestStatsArray[1,1]) then
          if frmMain.classcheckbox.IsChecked then
           tmp := tmp + ' The CSEM at the cutscore of '+formatfloat(prec,rawcut)+
                          ' equaled ' + formatfloat(prec,sqrt(lCsemArray[round(rawcut),2])) + '.';
        if (rawcut < 0) or (rawcut > FTestStatsArray[1,1]) then
          tmp := tmp + ' The scaled number correct cut-score of '+formatfloat(prec,rawcut)+' is outside of the number correct score range, the CSEM cannot be computed.';
        if rawcut < 0 then
      ...
      AddText('');           //is this an extra blank line above the quantile plot?
      ...
      for FItem := 1 to FNumberOfItems do
        ...
        begin
          ...
          begin
            DrawItemGraph(FGraphBitmap, FItem, lItemScaleArray, lItemIDArray, lItemInfoArray[FItem,1],lItemInfoArray[FItem,3]);
          ...
          PrintItemTables(prec, FItem,lItemInfoArray,lItemIDArray,
            lItemScaleArray,lNKeyArray,lItemInfoArray[FItem,3],lDifarray);
  ...
  //              ErrorMessage:=(E.Message, mtError, [mbOK], 0);
    ...
    begin
      ...
      frmMain.mLog.lines.Add(inttostr(FNumberOfItems) + ' items and ' + inttostr(FNumberOfExaminees) + ' examinees were processed');
...
begin
  ...
  try
    ...
    lMismatch:= False;
    lAllPilot:= True; //if all items are pilot, then program will stop and give an error
    lAllPC:= True;
    lZeroOk:= True;
    FAllOk:= True;
    lInputOk:= True;
    ...
    for i:= 1 to 4 do FNumMult[i]:= 0;
    for i:= 1 to 4 do FNumRate[i]:= 0;
    for i:= 1 to 100 do FDomMult[i]:= 0;
    for i:= 1 to 100 do FDomRate[i]:= 0;
    for i:= 1 to Length(datacontrolerror) do datacontrolerror[i]:= False;
    for i:= 1 to Length(patharray) do patharray[i]:= #0;
    for i:= 1 to Length(readformat) do readformat[i]:= #0;
    ...
    begin
      ...
      FNAChar:= lNAString[1];
      FMinP:= GetFloatValue(frmMain.MinPBox.Text);// StrToFloatDef(frmMain.MinPBox.Text, 0);
      FMaxP:= GetFloatValue(frmMain.MaxPBox.Text);// StrToFloatDef(frmMain.MaxPBox.Text, 0);
      FMinR:= GetFloatValue(frmMain.MinRpbisBox.Text);// StrToFloatDef(frmMain.MinRpbisBox.Text, 0);
      FMaxR:= GetFloatValue(frmMain.MaxRpbisBox.Text);// StrToFloatDef(frmMain.MaxRpbisBox.Text, 0);
      FMaxMean:= GetFloatValue(frmMain.MaxMeanBox.Text);// StrToFloatDef(frmMain.MaxMeanBox.Text, 0);
      FMinMean:= GetFloatValue(frmMain.MinMeanBox.Text);// StrToFloatDef(frmMain.MinMeanBox.Text, 0);
      lTypeOk:= True;
      lAltOk:= True;
      lDomOk:= True;
      lEndLine:= True;
      ...
      begin
        ...
        Setlength(lResponseArray, (FNumberOfExaminees + 10), (FNumberOfItems + 10));  //: Array [1..1000000,1..10000] of char;
        Setlength(lItemIDArray, (FNumberOfItems + 10));             //: Array [1..10000] of string;
        Setlength(lExamineeIDArray, (FNumberOfExaminees + 10));     //: Array [1..1000000] of string;
        Setlength(lItemInfoArray, (FNumberOfItems + 10), 10);    //5 Extra columns for multikeys
        Setlength(lItemScaleArray, (FNumberOfItems + 10), 3);
        Setlength(lRankingArray, (FNumberOfExaminees + 10), 3);   {1 = ranking, 2 = FGroup}
        ...
        begin
          ...
          SetLength(lDifRankArray, Succ(FNumberOfExaminees), 3); //1 =total rank, 2=FGroup
        end;
        ...
        if lIDBegin = 0 then lIDBegin:= 1;

        lColBox:= StrToIntDef(frmMain.itemColumnBox.Text, 0);
        ...
        begin
          ...
          repeat
            ...
            if i = 1 then vv:= CharCode(lChar);
//            if vv < 100 then INC(lIDSpace);
            If (vv < 100) and (lChar <> lTabChar) then lIDsArray[lIDSpace]:= lChar;
            If (lChar = lTabChar) and (lIDSpace = 1) then lIDsArray[lIDSpace]:= ' ';
            if vv < 100 then INC(lIDSpace);
          ...
          if (lChar = controldelim) then lKeyOk:= False;
          ...
          begin
            lItemInfoArray[FNumberItemsSoFar, lTempInt]:= UpCase(lChar);    //Multi FKey
            inc(lTempInt);
            inc(NumKey);
          ...
          for j:= succ(k) to 50 do
            readformat[j]:= chr(0);  // fills all remaining spots with Null char

          ContInt:= PWideChar(@readformat);    //Converts ReadFormat (array of Char) to string named ContInt
          lTempInt:= StrToInt(contint);
          ...
          if FNumberItemsSoFar = 1 then FStoreDomain[1]:= temp1;
          if FNumberItemsSoFar = 1 then lItemScaleArray[FNumberItemsSoFar,2]:= 1;
          ...
          Read(lItemInfoFile, lChar);
          lItemInfoArray[FNumberItemsSoFar, 2]:= UpCase(lChar);    {Scored}

          If UpperCase(lChar) = 'Y' then inc(FScoredItems);
          if UpperCase(lChar) = 'Y' then lAllPilot:= False;
          if UpperCase(lChar) = 'P' then inc(FPreTest);
          ...
          if UpperCase(lChar) <> 'P' then lAllPC:= False;
          if (lItemInfoArray[FNumberItemsSoFar,3] <> 'R') and (lItemInfoArray[FNumberItemsSoFar,3] <> 'M') and (lItemInfoArray[FNumberItemsSoFar,3] <> 'P') then
            lEndLine:= False;

          if UpperCase(lChar) = 'P' then IsZero:= True;
          if lItemInfoArray[FNumberItemsSoFar,3] <> 'M' then FIsRate:= True;
          if lItemInfoArray[FNumberItemsSoFar,3] = 'M' then FIsMult:= True;
          if (lItemInfoArray[FNumberItemsSoFar,3] = 'M') and (lItemInfoArray[FNumberItemsSoFar,2] = 'Y') then lScoredMult:= True;
          if (lItemInfoArray[FNumberItemsSoFar,3] <> 'M') and (lItemInfoArray[FNumberItemsSoFar,2] = 'Y') then ScoredRate:= True;

          if lItemInfoArray[FNumberItemsSoFar,2] = 'Y' then
          begin
            if lItemInfoArray[FNumberItemsSoFar,3] <> 'M' then FNumRate[1]:= FNumRate[1] + 1;
            if lItemInfoArray[FNumberItemsSoFar,3] <> 'M' then FDomRate[lItemScaleArray[FNumberItemsSoFar,2]]:= FDomRate[lItemScaleArray[FNumberItemsSoFar,2]] + 1;
            if lItemInfoArray[FNumberItemsSoFar,3] = 'M' then FNumMult[1] := FNumMult[1] + 1;
             if lItemInfoArray[FNumberItemsSoFar,3] = 'M' then FDomMult[lItemScaleArray[FNumberItemsSoFar,2]]:= FDomMult[lItemScaleArray[FNumberItemsSoFar,2]] + 1;
          end;
          if lItemInfoArray[FNumberItemsSoFar,2] <> 'N' then
          begin
            if lItemInfoArray[FNumberItemsSoFar,3] <> 'M' then FNumRate[2]:= FNumRate[2] + 1;
            if lItemInfoArray[FNumberItemsSoFar,3] = 'M' then FNumMult[2]:= FNumMult[2] + 1;
          end;
          if lItemInfoArray[FNumberItemsSoFar,2] = 'P' then
          begin
            if lItemInfoArray[FNumberItemsSoFar,3] <> 'M' then FNumRate[3]:= FNumRate[3] + 1;
            if lItemInfoArray[FNumberItemsSoFar,3] = 'M' then FNumMult[3]:= FNumMult[3] + 1;
          end;
          if lItemInfoArray[FNumberItemsSoFar,2] = 'N' then begin
            if lItemInfoArray[FNumberItemsSoFar,3] <> 'M' then FNumRate[4]:= FNumRate[4] + 1;
            if lItemInfoArray[FNumberItemsSoFar,3] = 'M' then FNumMult[4]:= FNumMult[4] + 1;
          end;
        ...
        CloseFile(lItemInfoFile);   //End of reading in the control file with 6 columns

        {-----error trapping # of items--------}
        if not frmMain.delimitbox.IsChecked then
        begin
         for lRow:= 1 to FNumberOfItems + pred(StrToIntDef(frmMain.itemColumnBox.Text, 0)) do
         begin
           Read(lDataMatrixFile,lChar);

           if (lChar = chr(48)) and (lRow > StrToIntDef(frmMain.itemColumnBox.Text, 0)) and
              (lItemInfoArray[lRow - succ(StrToIntDef(frmMain.itemColumnBox.Text, 0)), 3] <> 'P') and
              (chr(48) <> FOmitChar) and (chr(48) <> FNAChar)
           then
             lZeroOk:= False;

           if lZeroOk = False then FAllOk:= False;
           if lChar = chr(13) then FAllOk:= False;
         end;
         ...
         if not lKeyOk then frmMain.mLog.Lines.Add('ERROR! The key column has missing values');
         if not lAltOk then frmMain.mLog.Lines.Add('ERROR! The number of alternatives column includes a non-numeric character');
         if not lDomOk then frmMain.mLog.Lines.Add('ERROR! The domain column includes a non-numeric character');
         if not lEndLine then frmMain.mLog.Lines.Add('ERROR! Check the item type column, an unrecognized character was identified'+#13+ 'Item type includes M = Multiple Choice, R = Rating Scale, P = Partial Credit');
         if not lZeroOk then frmMain.mLog.Lines.Add('At least one valid item response of 0 was identified'+#13+'The item control file specified that item responses began at 1 for that item');
         Application.ProcessMessages;
      end;
      ...
      if not lAltOk or not lDomOk or not lEndLine then FAllOk:= False;
      ...
      begin
        ...
        if not FAllOk then ErrorMessage:= ('Please select an input file with an ITEMAN 3 Header');
        ...
        begin
          ...
          if frmMain.Is_Demo then
            if FNumberOfItems > 100 then FNumberOfItems:= 100;

          //Define all dynamic arrays - with some extra room just in case
          SetLength(lResponseArray, (FNumberOfExaminees + 10), (FNumberOfItems + 10));  //: Array [1..1000000,1..10000] of char;
          SetLength(lItemIDArray, (FNumberOfItems + 10));             //: Array [1..10000] of string;
          SetLength(lExamineeIDArray, (FNumberOfExaminees + 10));     //: Array [1..1000000] of string;
          SetLength(lItemInfoArray, (FNumberOfItems + 10), 10);    //Extra columns for multikeys
          SetLength(lItemScaleArray, (FNumberOfItems + 10), 3);
          SetLength(lRankingArray, (FNumberOfExaminees + 10), 3);   {1 = ranking, 2 = FGroup}
          SetLength(lReduceArray, (FNumberOfItems + 10), FNumCut + 1);
          SetLength(lSplitScore1, (FNumberOfExaminees + 2), 7);
          ...
          if frmMain.difcheckbox.IsChecked then
            SetLength(lDifRankArray, FNumberOfExaminees + 1, 3); //1 =total rank, 2=FGroup
          ...
          begin
            ...
            if lItemInfoArray[lRow,3] = 'R' then FIsRate:= True;
            if lItemInfoArray[lRow,3] = 'M' then FIsMult:= True;
            lItemIDArray[lRow]:= inttostr(lRow); //setting lItemIDArray to FItem sequence number
          end;
          ...
          begin
            ...
            if UpperCase(lChar) <> 'N' then
              lItemInfoArray[lRow, 2]:= 'Y'
            else
             lItemInfoArray[lRow, 2]:= 'N';

            if UpperCase(lChar) = 'Y' then FScoredItems := FScoredItems + 1;
            if UpperCase(lChar) = 'Y' then lAllPilot := False;
            if UpperCase(lChar) = 'P' then FPreTest := FPreTest + 1;

            if lItemInfoArray[lRow,2] = 'Y' then
            begin
              if lItemInfoArray[lRow,3] <> 'M' then FNumRate[1] := FNumRate[1] + 1;
              if lItemInfoArray[lRow,3] = 'M' then FNumMult[1] := FNumMult[1] + 1;
              if lItemInfoArray[lRow,3] <> 'M' then FDomRate[1] := FDomRate[1] + 1;
              if lItemInfoArray[lRow,3] = 'M' then FDomMult[1] := FDomMult[1] + 1;
            end;

            if lItemInfoArray[lRow,2] <> 'N' then
            begin
              if lItemInfoArray[lRow,3] <> 'M' then FNumRate[2] := FNumRate[2] + 1;
              if lItemInfoArray[lRow,3] = 'M' then FNumMult[2] := FNumMult[2] + 1;
            end;

            if lItemInfoArray[lRow,2] = 'P' then
            begin
              if lItemInfoArray[lRow,3] <> 'M' then FNumRate[3] := FNumRate[3] + 1;
              if lItemInfoArray[lRow,3] = 'M' then FNumMult[3] := FNumMult[3] + 1;
            end;

            if lItemInfoArray[lRow,2] = 'N' then
            begin
              if lItemInfoArray[lRow,3] <> 'M' then FNumRate[4] := FNumRate[4] + 1;
              if lItemInfoArray[lRow,3] = 'M' then FNumMult[4] := FNumMult[4] + 1;
            end;

            if (lItemInfoArray[lRow,3] = 'M') and (lItemInfoArray[lRow,2] = 'Y') then lScoredMult := True;
            if (lItemInfoArray[lRow,3] <> 'M') and (lItemInfoArray[lRow,2] = 'Y') then ScoredRate := True;
          ...
          begin
            ...
            if (lChar = chr(48)) and (chr(48) <> FOmitChar) and (chr(48) <> FNAChar) and (lRow > lExamineeIDColumns) then
            begin
              lZeroOk:= False;
              FAllOk:= False;
            end;
          ...
          if not FAllOk and lZeroOk and not lAllPilot then
            frmMain.mLog.Lines.add('Please check the number of items or number of ID columns specified in the ITEMAN 3 Header');
          if not FAllOk and not lZeroOk then
            frmMain.mLog.Lines.add('Valid item responses of 0 were found. The ITEMAN 3 Header does not support item responses that begin at 0.');

          if mrf1 <> 'GUI' then
          begin
            if not FAllOk then frmMain.mLog.Lines.add('ERROR! Check the number of items/ID columns in the ITEMAN 3 Header');
            if not lZeroOk then frmMain.mLog.Lines.add('ERROR! Item responses cannot begin at 0 when an ITEMAN 3 Header is used');
            if not FAllOk then frmMain.mLog.Lines.add('');
            if not FAllOk and lAllPilot and frmMain.cbDataMatrixFile.IsChecked then
          ...
          Reset(lDataMatrixFile);   //resetting matrix so it begins with first FItem
      ...
      if lAllPilot then FAllOk:= False;
      if not FAllOk and lAllPilot and not frmMain.cbDataMatrixFile.IsChecked then
        frmMain.mLog.Lines.add('There were no scored items specified in the item control file. ' + #13 + 'Please check the control file and make sure scored items are coded as Y');
      ...
      begin
        ...
        lMaxExamId:= 0; //maximum examinee id chars for delimited input, for correct output file
...
//          avc := 0;
          ...
          begin
            ...
            begin     //18-1 error trapping because lIDBegin will go negative!
              ...
              if lGroupCodeColumn < lIDBegin then  //if FGroup FCode appears BEFORE ID, parse it out, then Read to begin of ID
              begin
                for lColumn:= 1 to pred(lGroupCodeColumn) do
                  Read(lDataMatrixFile, lChar);

                if frmMain.difcheckbox.IsChecked then Read(lDataMatrixFile,lChar);
                if frmMain.difcheckbox.IsChecked then lResponseArray[lRow,0] := UpCase(lChar);  //DIF FCode
                for lColumn := 1 to lIDBegin - lGroupCodeColumn - 1 do
                  Read(lDataMatrixFile,lChar); //getting to begin of Real ID
              ...
              if (lGroupCodeColumn > lIDBegin) and (lGroupCodeColumn < (lIDBegin+lExamineeIDColumns)) then //if difcode is embedded in examinee ID, parse it out!
              begin
                idread := 0;
                ...
                begin
                  inc(idread);
                  Read(lDataMatrixFile, lChar);
                  lReadIDArray[idread] := lChar;                 {need to Reset to all nulls?}
                end;

                if frmMain.difcheckbox.IsChecked then Read(lDataMatrixFile,lChar);
                if frmMain.difcheckbox.IsChecked then lResponseArray[lRow,0] := UpCase(lChar);
                ...
                for lColumn:= 1 to (lColBox - lIDBegin - lExamineeIDColumns) do
                  Read(lDataMatrixFile,lChar); //getting to correct Data lColumn
              end;

              if (lGroupCodeColumn > lIDBegin) and (lGroupCodeColumn > (lIDBegin+lExamineeIDColumns-1)) then //if difcode appears after examinee ID, Read in ID first
              begin
                { Read ID data}
                for lColumn := 1 to lExamineeIDColumns do  //18-1-2
                begin
                  Read(lDataMatrixFile, lChar);
                  lReadIDArray[lColumn] := lChar;                 {need to Reset to all nulls?}
                end; //18-1-2
            ...
            if frmMain.difcheckbox.IsChecked then
              ...
              if lResponseArray[lRow,0] = UpCase(frmMain.group2code.Text[1]) then
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if lChar <> delim1 then lReadIDArray[lColumn] := lChar;
              end; //18-1-2

              if lColumn > lMaxExamId then lMaxExamId := lColumn;
              lExamineeID := PWideChar(@lReadIDArray);
              lExamineeIDArray[lRow] := lExamineeID;
            end else
            begin
              lExamineeIDArray[lRow] := inttostr(lRow); // so examinee ID isn't null! and equals row number
              x := power(10,lMaxExamId);

              if 0 = (lRow mod round(x)) then
                inc(lMaxExamId); // this increments maximum # characters for examinee ID
            end;
            ...
            begin
              ...
              if UpperCase(lChar) = UpCase(frmMain.group2code.Text[1]) then
                lResponseArray[lRow,0] := '2'
              else
                lResponseArray[lRow,0] := '0';
          ...
          begin
            ...
            begin  //trapping null responses
              ...
              if not lIsDelim then
                Read(lDataMatrixFile, lChar, lChar)
              else
                Read(lDataMatrixFile, lChar);

              if lChar = delim1 then lIsDelim:= True else lIsDelim:= False;
              if lIsDelim then lChar:= UpCase(FNAChar);
            end;
...
//              avc:= lRow;
            ...
            begin
              ...
              if vv = Succ(lItemScaleArray[lColumn,1]) then
                lResponseArray[lRow,lColumn]:= UpCase(FOmitChar); //setting unrecognized chars to omit
          ...
          if errno = 0 then ReadLn (lDataMatrixFile);
        ...
        for lColumn := 1 to FNumberOfItems do if datacontrolerror[lColumn] = True then
        begin
          if (isnumeric(lItemInfoArray[lColumn,1])) and (matchwarning = '') then matchwarning := 'numeric';
          if (isalphabetic(lItemInfoArray[lColumn,1])) and (matchwarning = '') then matchwarning := 'alphabetical';
          if (isnumeric(lItemInfoArray[lColumn,1])) and (matchwarning[1] = 'a') then matchwarning := 'alphabetical or numeric';
          if (isalphabetic(lItemInfoArray[lColumn,1])) and (matchwarning[1] = 'n') then matchwarning := 'numeric or alphabetical';
          if (isnumeric(lItemInfoArray[lColumn,1])) and (revmatchwarning = '') then revmatchwarning := 'alphabetical';
          if (isalphabetic(lItemInfoArray[lColumn,1])) and (revmatchwarning = '') then revmatchwarning := 'numeric';
          if (isnumeric(lItemInfoArray[lColumn,1])) and (revmatchwarning[1] = 'n') then revmatchwarning := 'numeric or alphabetical';
          if (isalphabetic(lItemInfoArray[lColumn,1])) and (revmatchwarning[1] = 'a') then revmatchwarning := 'alphabetical or numeric';
        ...
        if i <> 0 then
          frmMain.mLog.Lines.add('Check the data matrix file, examinee '+inttostr(i)+ ' did not respond to all '+ inttostr(FNumberOfItems)+ ' items.');
        ...
        if FIsRate then frmMain.progressbar.max:= frmMain.progressbar.max + FNumberOfItems;
        if FIsMult then frmMain.progressbar.max:= frmMain.progressbar.max + FNumberOfItems;
        if FNumDomains > 1 then frmMain.progressbar.max:= frmMain.progressbar.max + 2 * FNumberOfItems * FNumDomains;

        frmMain.progressbar.Value:= 0;

        if not FRespOK then
          frmMain.mLog.Lines.add('At least one unidentified response character was identified' + #13 + ' and will be scored as incorrect.');
          ...
          begin
            ...
            begin
              ...
              if frmMain.ScoredMatrixBox.IsChecked then
                ...
                write(lControlText,chr(9)+lItemInfoArray[lColumn,1]+chr(9)+inttostr(lItemScaleArray[lColumn,1])+chr(9)+inttostr(lItemScaleArray[lColumn,2])+chr(9)+lItemInfoArray[lColumn,2]);

              if frmMain.ScoredMatrixBox.IsChecked then writeln(lControlText,chr(9)+'P');
              if not frmMain.ScoredMatrixBox.IsChecked then writeln(lControlText,chr(9)+'M');
            end; //if cbDataMatrixFile.IsChecked

            if (lRow = 1) and (frmMain.SaveControl.IsChecked) and (lItemInfoArray[lColumn,3] = 'R') then
            begin
              write(lControlText,'Item '+inttostr(i));
               write(lControlText,chr(9)+ '+' +chr(9)+inttostr(lItemScaleArray[lColumn,1])+chr(9)+inttostr(lItemScaleArray[lColumn,2])+chr(9)+lItemInfoArray[lColumn,2]);
              ...
              begin
                if lItemInfoArray[lColumn,3] = 'M' then
                  ...
                  begin
                    ...
                    begin {Correct}
                      if frmMain.ScoredMatrixBox.IsChecked then write(lScoredMatrixFile, '1');
                      if lItemInfoArray[lColumn,2] <> 'N' then lScoreArray[lRow,4] := lScoreArray[lRow,4] + 1;   {all-FItem score}

                      if lItemInfoArray[lColumn,2] = 'Y' then   {FItem is scored}
                        lScoreArray[lRow,1] := lScoreArray[lRow,1] + 1;    {scored-FItem score}

                      if lItemInfoArray[lColumn,2] = 'P' then   {FItem is FPreTest}
                        lScoreArray[lRow,5] := lScoreArray[lRow,5] + 1;      {FPreTest-FItem score}
                    end else
                    if frmMain.ScoredMatrixBox.IsChecked then
                  ...
                  begin
                    if lItemInfoArray[lColumn,2] <> 'N' then
                      lScoreArray[lRow,4]:= lScoreArray[lRow,4] + convertresponsetointeger(lResponseArray[lRow,lColumn],lItemInfoArray,lColumn,lItemScaleArray[lColumn,1]);
                    if lItemInfoArray[lColumn,2] = 'Y' then
                      lScoreArray[lRow,1] := lScoreArray[lRow,1] + convertresponsetointeger(lResponseArray[lRow,lColumn],lItemInfoArray,lColumn,lItemScaleArray[lColumn,1]);
                    if lItemInfoArray[lColumn,2] = 'P' then
                      lScoreArray[lRow,5] := lScoreArray[lRow,5] + convertresponsetointeger(lResponseArray[lRow,lColumn],lItemInfoArray,lColumn,lItemScaleArray[lColumn,1]);
                    if frmMain.ScoredMatrixBox.IsChecked then
                      write(lScoredMatrixFile, convertresponsetointeger(lResponseArray[lRow,lColumn],lItemInfoArray,lColumn,lItemScaleArray[lColumn,1]));
            ...
            if (frmMain.ScoredMatrixBox.IsChecked) and (frmMain.InclOmit.IsChecked = False) and (lItemInfoArray[lColumn,3] = 'M') then
              if lResponseArray[lRow,lColumn] = FOmitChar then
                write(lScoredMatrixFile,'0');

            if (frmMain.ScoredMatrixBox.IsChecked) and (frmMain.InclNR.IsChecked = False) and (lItemInfoArray[lColumn,3] = 'M') then
              if lResponseArray[lRow,lColumn] = FNAChar then
                write(lScoredMatrixFile,'0');
        ...
        ScoreGroups(lRankingArray,lReduceArray,lScoreArray,lItemInfoArray,lResponseArray);  //split sample into scoregroups
        SumStats(lItemInfoArray,lScoreArray);  //compute mean,sd,var,min,and max for all scales
        OptionStats(lItemInfoArray,lResponseArray,lItemScaleArray,lNKeyArray,lScoreArray);  //option rpbis, rbis, means , FOmch defined here
        ...
        if FNumDomains > 1 then SetLength(lSplitDomain1, FNumberOfExaminees + 2, 1 + FNumDomains * 3);
        if FNumDomains > 1 then SetLength(lSplitDomain2, FNumberOfExaminees + 2, 1 + FNumDomains * 3);

        Reliability(lSplitScore1,lSplitScore2,lSplitDomain1,lSplitDomain2,lItemScaleArray,lItemInfoArray,lResponseArray);
        ...
        begin
          ...
          if (FPreTest > 0) and (FTestStatsArray[2,6] <= 1) then FTestStatsArray[2,9] := FTestStatsArray[2,3]*sqrt(1-FTestStatsArray[2,6]);
          if (FPreTest > 0) and (FTestStatsArray[3,6] <= 1) then FTestStatsArray[3,9] := FTestStatsArray[3,3]*sqrt(1-FTestStatsArray[3,6]);
...
////          Exit;
      ...
      if not usemrf then lOutputFileName:= '"' + frmMain.docxoutput + '"';
      if usemrf then lOutputFileName:= '"' + mrfname + '"';
...
begin
   foundint := false;
   for i := 0 to 9 do if FCh = chr(48+i) then foundint := true;
   if (FCh = chr(43)) or (FCh = chr(45)) then foundint := true;  //don't error trap is control file is +/- poly
   Result:= foundint;
END;
...
begin
   foundalp := false;
   for i := 0 to 14 do if FCh = chr(65+i) then foundalp := true;
   if (FCh = chr(43)) or (FCh = chr(45)) then foundalp := true;  //don't error trap is control file is +/- poly
   Result:= foundalp;
END;

{This is needed because the response number is used as an index}
function TItemanCalculation.ConvertResponseToInteger(FCh: char; iteminfoarray : TChar2Array; FItem, nalt : integer): integer;
var
  g: integer;
begin
  Result:= Succ(nalt);
  ...
  begin
    ...
    if ((FCh = chr(48)) or (FCh = chr(26))) and ((FCh <> FOmitChar) and (FCh <> FNaChar)) then FAllOk:= false;
  ...
  if (result = Succ(nalt)) and (FOmitChar <> chr(48)) and (FNaChar <> chr(48)) then FRespOk:= false; //chr(48) is 0
end;

function TItemanCalculation.RespToInt(FCh : char; iteminfoarray : TChar2Array; FItem : integer; itemscalearray : TIntegerArray): Integer;
var
  aj: integer;
begin
  if iteminfoarray[FItem,3] = 'P' then
  ...
  if FCh = '0' then Result:= 1 - aj else
  if (FCh = '1') or (FCh = 'A') then Result := 2-aj else
  if (FCh = '2') or (FCh = 'B') then Result := 3-aj else
  if (FCh = '3') or (FCh = 'C') then Result := 4-aj else
  if (FCh = '4') or (FCh = 'D') then Result := 5-aj else
  if (FCh = '5') or (FCh = 'E') then Result := 6-aj else
  if (FCh = '6') or (FCh = 'F') then Result := 7-aj else
  if (FCh = '7') or (FCh = 'G') then Result := 8-aj else
  if (FCh = '8') or (FCh = 'H') then Result := 9-aj else
  if (FCh = '9') or (FCh = 'I') then Result := 10-aj else
  if FCh = 'J' then Result := 11-aj else if FCh = 'K' then Result := 12-aj else
  if FCh = 'L' then Result := 13-aj else if FCh = 'M' then Result := 14-aj else;
  if FCh = 'N' then Result := 15-aj else if FCh = 'O' then Result := 16-aj;
  if upcase(FCh) = upcase(FOmitChar) then Result:= Succ(itemscalearray[FItem,1])
  else if upcase(FCh) = upcase(FNaChar) then Result:= itemscalearray[FItem,1] + 2;
end;

procedure TItemanCalculation.mantelH(item : integer; responsearray, iteminfoarray : TChar2Array; difrankarray : TIntegerArray; difarray : TRealArray);
var i, j, si, row, numcut : integer;
    alphas : array[1..7] of Real;
    contingency : array[1..2,1..2] of Real;
    rk,sk,r,s,pk,qk,temp1,temp2,temp3 : Real; //terms for the SE of M-H statistics
begin
  difarray[item,1]:= 0;
  ...
  begin
    ...
    for row:= 1 to FNumberOfExaminees do
      ...
      begin
        if (responsearray[row,item] = iteminfoarray[item,1]) and (responsearray[row,0] = '1') then contingency[1,1] := contingency[1,1] + 1;
        if (responsearray[row,item] <> iteminfoarray[item,1]) and (responsearray[row,0] = '1') then contingency[1,2] := contingency[1,2] + 1;
        if (responsearray[row,item] = iteminfoarray[item,1]) and (responsearray[row,0] = '2') then contingency[2,1] := contingency[2,1] + 1;
        if (responsearray[row,item] <> iteminfoarray[item,1]) and (responsearray[row,0] = '2') then contingency[2,2] := contingency[2,2] + 1;
      end;
  ...
  difarray[item,1] := difarray[item,1]/difarray[item,2]; //alpha M-H
  difarray[item,2] := -2.35*ln(difarray[item,1]);
  difarray[item,3] := sqrt(0.5*((temp1/(r*r))+(temp2/(r*s))+(temp3/(s*s)))); //Std error of log(alpha M-H)
  difarray[item,4] := -1 * ln(difarray[item,1]) / difarray[item,3]; //multiply by -1 so - = ref higher, + = focal higher
  difarray[item,5] := cdfn(difarray[item,4]); //FP-value for DIF FZ-test

  for si := 1 to numcut do
    difarray[item,si+5] := alphas[si]; //sub-FGroup alphas, useful for identifying crossing vs uniform DIF
end;

function TItemanCalculation.cdfn(x: single) : single; // Cdfn integrates the normal density function to the deviate x.
var
  y: single;
begin
  y:= x * 0.707107;
...
procedure TItemanCalculation.DIFgroups(difrankarray,scorearray : TIntegerArray; iteminfoarray, responsearray : TChar2Array );
var
...
begin
  ...
  for Row := 1 to FNumberOfExaminees do if responsearray[row,0] <> '0' then
  begin
    numscoreshigher := 0;
    inc(FNumberForDif);
...
procedure TItemanCalculation.ScoreGroups(RankingArray, ReduceArray, scorearray : TIntegerArray; iteminfoarray, responsearray : TChar2Array );
var
...
begin
  ...
  for row:= 1 to FNumberOfExaminees do
    for column:= 1 to FNumberOfItems do if iteminfoarray[column,2] <> 'N' then
    begin
...
begin
  ...
  if FNumberOfExaminees > 1 then FTestStatsArray[2,3] := sqrt(ScoreSS1/(FNumberOfExaminees - 1));
  if FNumberOfExaminees > 1 then FTestStatsArray[1,3] := sqrt(ScoreSS2/(FNumberOfExaminees - 1));
  if FNumberOfExaminees > 1 then FTestStatsArray[3,3] := sqrt(ScoreSS3/(FNumberOfExaminees - 1));
  ...
  for Row := 1 to FNumberOfExaminees do
  begin
    If ScoreArray[Row,4] < temp1 then temp1 := ScoreArray[Row,4];
    if scorearray[row,1] < temp2 then temp2 := scorearray[row,1];
    if scorearray[row,5] < temp3 then temp3 := scorearray[row,5];
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        rescaled := rescaled + (StrToFloatDef(frmMain.newmeanbox.Text, 0) - (StrToFloatDef(frmMain.newsdbox.Text, 0) / FTestStatsArray[1,3]) * FTestStatsArray[1,2]);
      end;

      if frmMain.linearscale.IsChecked then
        rescaled := (scorearray[row,1] * StrToFloatDef(frmMain.slopebox.Text, 0)) + StrToFloatDef(frmMain.interceptbox.Text, 0);
    ...
    for row := 1 to FNumberOfExaminees do
    begin
      FScoreSS:= FScoreSS + (FTestStatsArray[4,2] - FScaledScoreArray[Row])*(FTestStatsArray[4,2] - FScaledScoreArray[Row]);
    end;
  ...
  begin
    ...
    for row := 1 to FNumberOfExaminees do
      FScoreSS := FScoreSS + (FTestStatsArray[5,2] - Fexternalscorearray[Row])*(FTestStatsArray[5,2] - Fexternalscorearray[Row]);
...
procedure TItemanCalculation.OptionStats(iteminfoarray, responsearray : TChar2Array; itemscalearray,nkeyarray,scorearray : TIntegerArray);
var
...
begin
  ...
  for Column:= 1 to FNumberOfItems do
    ...
    begin
      ...
      FItemStatsArray[column,2] := temp0 / (FItemN+1E-20);  //store the item mean, independent of item type
      FItemStatsArray[column,1] := FItemN; {record in overall item stat array}

      if iteminfoarray[column,3] <> 'M' then
      begin
        if FItemStatsArray[column,2] < FMinItem1 then FMinItem1 := FItemStatsArray[column,2];
        if FItemStatsArray[column,2] > FMaxItem1 then FMaxItem1 := FItemStatsArray[column,2];
        if itemscalearray[column,1] > maxcategory then maxcategory := itemscalearray[column,1];
      end;
  ...
  for i:= 1 to FNumberOfItems do if iteminfoarray[i,2] <> 'N' then
  begin
    frmMain.progressbar.Value:= frmMain.progressbar.Value + 1;
    Application.ProcessMessages;

    if iteminfoarray[i,3] = 'P' then FWts:= 1 else FWts:= 0;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if (iteminfoarray[i,3] = 'M') and (upcase(FCh) = upcase(ItemInfoArray[i,1])) then {If the response is the FKey}
              if ItemInfoArray[i,2] = 'Y' then  {If item is scored}
                ScoreArray[Row,3] := ScoreArray[Row,3] - 1;    {Subtracts the option 1/0 indicator}
            ...
            if (iteminfoarray[i,3] <> 'M') and (iteminfoarray[i,2] = 'Y') then
              scorearray[row,3] := scorearray[row,3] - convertresponsetointeger(responsearray[row,i],iteminfoarray,i,itemscalearray[i,1]);
          end;
          ...
          begin
            ...
            if frmMain.cbExternalScoreFile.IsChecked then FRpbisCalcArray[j,1]:= FRpbisCalcArray[j,1] + FExternalScoreArray[Row];
            if not frmMain.cbExternalScoreFile.IsChecked then rawsum:= rawsum + ScoreArray[Row,1];
            if frmMain.cbExternalScoreFile.IsChecked then rawSum:= rawSum + FExternalScoreArray[Row];
          end;
        end;

        if frmMain.cbExternalScoreFile.IsChecked then exts := FExternalScoreArray[row] else exts := scorearray[row,1];
      ...
      begin
        ...
        if FResponseAsInt +FWts = j then
          FOptionSDArray[i, j]:= FOptionSDArray[i, j] + (exts - FOptionMeansArray[i,j]) * (exts - FOptionMeansArray[i,j]);

        if frmMain.cbExternalScoreFile.IsChecked then exts := FExternalScoreArray[row] else exts := scorearray[row,1]; //now full score
        if (responsearray[row,i] = FOmitChar) and (j = 1) then
          FOptionSDArray[i,16] := FOptionSDArray[i,16] + (exts-FOptionMeansArray[i,16])*(exts-FOptionMeansArray[i,16]);
        if (responsearray[row,i] = FNAChar) and (j = 1) then
          FOptionSDArray[i,17] := FOptionSDArray[i,17] + (exts-FOptionMeansArray[i,17])*(exts-FOptionMeansArray[i,17]);
      end;
      ...
      begin
        ...
        for row := 1 to FNumberOfExaminees do
          ...
          begin
            ...
            if upcase(iteminfoarray[i,1]) = upcase(FCh) then
              multisum := multisum + ((exts-multiMean)/(multiSD+1E-20))*((1-FP)/(sqrt(FP*(1-FP))+1E-20))
            else
            for k := 1 to nkeyarray[i,1]-1 do
              if upcase(iteminfoarray[i,k+3]) = upcase(FCh) then
                multisum := multisum + ((exts-multiMean)/(multiSD+1E-20))*((1-FP)/(sqrt(FP*(1-FP))+1E-20));

            if (upcase(iteminfoarray[i,1]) = upcase(FCh)) then correct := true;

            for k := 1 to nkeyarray[i,1]-1 do
              if upcase(iteminfoarray[i,k+3]) = upcase(FCh) then correct := true;

            if not correct then
              multisum := multisum + ((exts-multiMean)/(multiSD+1E-20))*((0-FP)/(sqrt(FP*(1-FP))+1E-20));
          end;
        ...
        If multibis > 1.0 then multibis := 1.0;      {cap}
        If multibis < -1.0 then multibis := -1.0;    {cap}
        FItemStatsArray[i,4] := multibis;
      end; //if nkeyarray > 1

      if FSpurN > 1 then   FSpurScoreSD := sqrt(FScoreSS/(FSpurN - 1));
      ...
      begin
        ...
        for row := 1 to FNumberOfExaminees do
          ...
          begin
            ...
            if ConvertResponseToInteger(ResponseArray[row,i],iteminfoarray,i,itemscalearray[i,1]) +FWts = j then
              FRpbisSum:= FRpbisSum + ((exts - FSpurScoreMean)/(FSpurScoreSD + 0.000000000000000000001)) * ((1-FP)/(sqrt(FP*(1-FP))+0.000000000000000000001))
            else
              FRpbisSum:= FRpbisSum + ((exts - FSpurScoreMean)/(FSpurScoreSD + 0.000000000000000000001)) * ((0-FP)/(sqrt(FP*(1-FP))+0.000000000000000000001));
          end;

        FOptionRpbisArray[i,j] := FRpbisSum/(FNforRpbis-1+1E-20);

        if (FOptionRpbisArray[i,j] > 1) then FOptionRpbisArray[i,j] := 1;
        ...
        If FOptionRbisArray[i,j] > 1.0 then FOptionRbisArray[i,j] := 1.0;      {cap}
        If FOptionRbisArray[i,j] < -1.0 then FOptionRbisArray[i,j] := -1.0;    {cap}
      end; //if FP > 0
    end;   {j = options}
      ...
      begin
        ...
        begin
            ...
            begin
              ...
              FRpbisSum:= FRpbisSum + ((exts-FOptionMeansArray[i,16])/(FSpurScoreSD+0.000000000000000000001)) * ((0-FP)/(sqrt(FP*(1-FP))+0.000000000000000000001));
            end;
        end;
        ...
        if FOptionRbisArray[i,16] > 1.0 then FOptionRbisArray[i,16] := 1.0;      {cap}
        if FOptionRbisArray[i,16] < -1.0 then FOptionRbisArray[i,16] := -1.0;    {cap}
      end; //if FP > 0
    ...
    begin
      ...
      for row := 1 to FNumberOfExaminees do
        ...
        begin
          ...
          if (frmMain.spurbox.IsChecked = false) or (iteminfoarray[column,2] = 'P') then FResponseAsInt := 0;
      ...
      for row := 1 to FNumberOfExaminees do
        ...
        begin
          ...
          TSD := TSD + ((scorearray[row,1]-FResponseAsInt) - TMean)*((scorearray[row,1]-FResponseAsInt) - TMean);
        end;

      if FItemN > 1 then TSD := sqrt(TSD/(FItemN-1));
      ...
      begin
        ...
        if (responsearray[row,column] = FOmitChar) and (responsearray[row,column] = FNAChar) then FResponseAsInt := 0;
        ...
        begin
          ...
          temp1:= temp1 + (FResponseAsInt - FItemStatsArray[column,2]) * (FResponseAsInt - FItemStatsArray[column,2]);
      ...
      if FItemN > 1 then   temp4 := temp4 / (FItemN - 1);
      if FItemN > 1 then   FItemStatsArray[column,6] := temp1 / (FItemN - 1);

      for row := 1 to itemscalearray[column,1] do
        SSbet := Ssbet + domcount[row] * (dommean[row]-(TMean-FItemStatsArray[column,2]))* (dommean[row]-(TMean-FItemStatsArray[column,2]));
      ...
      if FItemStatsArray[column,3] < FMinCorr then FMinCorr := FItemStatsArray[column,3];
...
procedure TItemanCalculation.PrintFirstSummary(prec: string;
  iteminfoarray: TChar2Array; itemidarray: TStringArray;
  itemscalearray: TIntegerArray; difarray: TRealArray);
var
  s: string;
...
//  AddSummary(frmMain.imgSumm.Bitmap);
  ...
  begin
    ...
    if (Fnumdomains = 1) and (Fpretest = 0) then summarytablerows := 2 else SummaryTableRows := 4;
    If ScaledCheckBox.IsChecked then SummaryTableRows := summarytablerows + 1;
    if (Fnumdomains > 1) and (scaledDomain.IsChecked) then summarytablerows := summarytablerows + Fnumdomains;
    If cbExternalScoreFile.IsChecked then SummaryTableRows := SummaryTableRows + 1;
    if (Fpretest = 0) and (Fnumdomains <> 1) then summarytablerows := summarytablerows - 1;
    if Fnumdomains <> 1 then SummaryTableRows := SummaryTableRows + FNumDomains;

    {Write summary statistics to rtf output AS A RICHVIEW TABLE}
    if Fisrate and Fismult then
      coladj := 1 else coladj := 0;
    ...
    TempInt := 4;
    if Fpretest = 0 then tempint := tempint - 2;
    ...
    begin
      if Fpretest > 0 then
        start := 5 else start := 4;
      if Fpretest > 0 then
        finish := Fnumdomains + 4 else finish := Fnumdomains + 3;
    end
    else
      begin
        if Fpretest > 0 then
          start := 4 else start := 3;
        if Fpretest > 0 then
          finish := Fnumdomains + 3 else finish := Fnumdomains + 2;
      end;
    ...
    if (Fnumdomains <> 1) and (scaleddomain.IsChecked = false)
      then finish := summarytablerows - 1;
    If ScaledCheckBox.IsChecked then
      begin
        if scaleddomain.IsChecked then inc(finish);
    ...
    if Fnumdomains <> 1 then
      begin
        domain := 0;
        if scaledDomain.IsChecked then for row := finish+1 to summarytablerows-1 do
          begin
            domain := domain + 1;
    ...
    SummaryTableRows := 2;
    if Fpretest > 0 then summarytablerows := summarytablerows + 2;
    if Fnumdomains > 1 then summarytablerows := summarytablerows + Fnumdomains;
   ...
   if Fnumdomains > 1 then for h := 1 to Fnumdomains do
    begin
    ...
    if (classcheckbox.IsChecked) then
      begin
        s := 'The cutscore on this exam was ' + classcutbox.Text + ', producing a pass rate of '+ formatfloat('0.0',100*Fproppassing)+'%.';
        if not scoredrate then
          s := s + '  The Livingston index of classification consistency at the cut-score was ' + formatfloat(prec,livingston())+'.';
        AddText(PWideChar(s));
        AddText('');
      end;
            ...
            begin
              ...
              for j := 1 to itemscalearray[row,1] do
                if (iteminfoarray[row,3] = 'M') and (Fitemstatsarray[row,6] <> 0) then
                 if Fitemstatsarray[row,3] < Foptionrpbisarray[row,j]
                   then checkkey := true;
              ...
              if (difcheckbox.IsChecked) and
                  (iteminfoarray[row,2] = 'Y') and (iteminfoarray[row,9] = 'D') then
                begin
...
begin
  ...
  for i := 1 to nbin do
  begin
    if (overrule = true) or (userange = true) then lowreal := groupedfreqarray[i,1];
    if (overrule = true) or (userange = true) then highreal := groupedfreqarray[i,2];
    if (overrule = false) and (userange = false) then
      begin
         low1 := trunc(groupedfreqarray[i,1]);
         high1 := trunc(groupedfreqarray[i,2]);
         if (groupedfreqarray[i,2] = high1) and (i <> nbin) then high1 := high1 - 1;  //because last bin always includes max
         if (low1 < groupedfreqarray[i,1]) and (i <> 1) then low1 := low1 + 1;  //because 1st bin always includes minimum
         lowreal := low1;
         highreal := high1;
      end;
   if (highreal - lowreal) < 1E-9 then smallrange := true else smallrange := false;
...
begin
  ...
  if min = -1 then userange := true else userange := false;
  if max-min < 20 then overrule := true else overrule := false;

  if (overrule = true) or (userange = true) then
   tbMatrix[0,0] := 'Score' else
   tbMatrix[0,0] := 'Range';
  tbMatrix[0,1] := 'Frequency';
  ...
  for i := 1 to nbin do begin
   if (overrule = true) or (userange = true) then lowreal := groupedfreqarray[i,1];
   if (overrule = true) or (userange = true) then highreal := groupedfreqarray[i,2];
   if (overrule = false) and (userange = false) then begin
        low1 := trunc(groupedfreqarray[i,1]);
        high1 := trunc(groupedfreqarray[i,2]);
        if (groupedfreqarray[i,2] = high1) and (i <> nbin) then high1 := high1 - 1;  //because last bin always includes max
        if (low1 < groupedfreqarray[i,1]) and (i <> 1) then low1 := low1 + 1;  //because 1st bin always includes minimum
...
procedure TItemanCalculation.PrintItemTables(const prec: string; item: integer;
  iteminfoarray: TChar2Array; itemidarray: TStringArray; itemscalearray,
  nkeyarray: TIntegerArray; start0: char; difarray: TRealArray);
var
  checkkey : boolean;
  PreStr, PreText : string;
  DIFInt,j, d,g, REO, i, k, SCO, row : integer;  //REO is reorder variable if polytomous codes are reversed.
  tbMatrix: TTableMatrix;
begin
   REO := 0;
  with frmMain do
  begin
     if plotsbox.IsChecked = false then i := 1 else i := 0; //no colors needed in tables
     if Fnumdomains > 1 then g := 2 else g := 0;
    ...
    if iteminfoarray[item,3] <> 'M' then
      begin
        if checkkey = true then prestr := ', ' else prestr := '';
    ...
    if (difcheckbox.IsChecked) and (iteminfoarray[item,2] = 'Y') and
        (iteminfoarray[item,9] = 'D') then
      begin
    ...
    if difcheckbox.IsChecked then DIFInt := 3 else DIFInt := 0;
    ...
    if Fnumdomains > 1 then
      begin
       tbMatrix[1,2] := FormatFloat(prec,FItemStatsArray[item,7]);  //domain rpbis/R
       tbMatrix[1,3] := FormatFloat(prec,FItemStatsArray[item,8]);  //domain rbis/eta
      end;
    ...
    if iteminfoarray[item,2] = 'Y' then
      PreStr := formatfloat(prec,FItemStatsArray[item,5]) else
      PreStr := 'NA';
    ...
    tbMatrix[0,0] := 'Option';
    if iteminfoarray[item,3] <> 'M' then j := 1 else j := 0;
    if iteminfoarray[item,3] <> 'M' then
      tbMatrix[0,1] := 'Weight';
    tbMatrix[0,1+j] := 'N';
    tbMatrix[0,2+j] := 'Prop.';
      ...
      begin
        if row <= itemscalearray[item,1] then
          begin
              ...
              begin
                ...
                if iteminfoarray[item,1] = '-' then
                  if iteminfoarray[item,3] = 'R' then
                    REO := (ItemScaleArray[item,1]+1-row) else
                    REO := row;
                if iteminfoarray[item,1] = '-' then
                  if iteminfoarray[item,3] = 'P' then
            ...
            if iteminfoarray[item,3] = 'M' then REO := row;
        ...
        if start0 = 'P' then SCO := 1 else SCO := 0;
        If (ConvertResponseToInteger(ItemInfoArray[item,1],iteminfoarray,item,itemscalearray[item,1]) = row-SCO) and
           (iteminfoarray[item,3] = 'M') then
             tbMatrix[row,8-i] := '**KEY**';
        if nkeyarray[item,1] > 1 then
          for k := 1 to nkeyarray[item,1]-1 do
            If (ConvertResponseToInteger(ItemInfoArray[item,3+k],iteminfoarray,item,itemscalearray[item,1]) = row-SCO) and
           (iteminfoarray[item,3] = 'M') then
    ...
    For row := 1 to ItemScaleArray[item,1] do
      if plotsbox.IsChecked then
        begin  //if no plotting, then colors not needed
          If row = 1 then tbMatrix[row,7+j] := 'Rust';
          If row = 2 then tbMatrix[row,7+j] := 'Navy';
          If row = 3 then tbMatrix[row,7+j] := 'Gold';
          If row = 4 then tbMatrix[row,7+j] := 'Gray';
          If row = 5 then tbMatrix[row,7+j] := 'LightBlue';
          If row = 6 then tbMatrix[row,7+j] := 'Navy';
          If row = 7 then tbMatrix[row,7+j] := 'Red' ;
          If row = 8 then tbMatrix[row,7+j] := 'Teal';
          If row = 9 then tbMatrix[row,7+j] := 'Purple';
        end; //for row loop

    AddTable('Option statistics', tbMatrix);
    if not tablebox.IsChecked then exit;
    ...
    for j := 1 to round(cutpoint.Text.ToDouble) do
      tbMatrix[0,j+1] := formatfloat('0',((j-1)/Fnumcut)*100)+'-'+formatfloat('0',(j/Fnumcut)*100)+'%';
    ...
    For row := 1 to ItemScaleArray[item,1] do
      begin
        ...
        if plotsbox.IsChecked then
          begin  //if no plotting, then colors not needed
            If row = 1 then tbMatrix[row,2+Fnumcut] := 'Rust';
            If row = 2 then tbMatrix[row,2+Fnumcut] := 'Navy';
            If row = 3 then tbMatrix[row,2+Fnumcut] := 'Gold';
            If row = 4 then tbMatrix[row,2+Fnumcut] := 'Gray';
            If row = 5 then tbMatrix[row,2+Fnumcut] := 'LightBlue';
            If row = 6 then tbMatrix[row,2+Fnumcut] := 'Navy';
            If row = 7 then tbMatrix[row,2+Fnumcut] := 'Red';
            If row = 8 then tbMatrix[row,2+Fnumcut] := 'Teal';
            If row = 9 then tbMatrix[row,2+Fnumcut] := 'Purple';
          end;
...
begin
  ...
  If FScoredItems > 1 then     //alpha for scored items
    FTestStatsArray[1,6]:= (FScoredItems/(FScoredItems-1))*(1-(lpsum[1]/(FTestStatsArray[1,3]*FTestStatsArray[1,3] + 0.000000000001)))
  else
    FTestStatsArray[1,6]:= 0;

  If Fvaliditems > 1 then     //alpha for all items
    FTestStatsArray[2,6]:= (Fvaliditems/(Fvaliditems-1))*(1-(lpsum[2]/(FTestStatsArray[2,3]*FTestStatsArray[2,3] + 0.000000000001)))
  else
    FTestStatsArray[2,6]:= 0;

  if FTestStatsArray[3,1] > 1 then     //alpha for pretest items
    FTestStatsArray[3,6]:= (FTestStatsArray[3,1]/(FTestStatsArray[3,1]-1))*(1-(lpsum[3]/(FTestStatsArray[3,3]*FTestStatsArray[3,3] + 0.000000000001)))
  else
...
procedure TItemanCalculation.EstimateKR20Without(ResponseArray, ItemInfoArray : TChar2Array; itemscalearray,nkeyarray,scorearray : TIntegerArray);
var
  lScoreSum: Real;
  k,i,j, row : integer;
begin
    ...
    begin
      ...
      begin
        ...
        if (FCh = ItemInfoArray[i,1]) and (iteminfoarray[i,3] = 'M') then    {If the response is the key}
          ScoreArray[Row,3] := ScoreArray[Row,3] - 1;    {Subtracts the 1/0 indicator}
        ...
        if nkeyarray[i,1] > 1 then
          ...
          begin
            ...
            if (FCh = ItemInfoArray[i,3+j]) and (iteminfoarray[i,3] = 'M') then j := j + 1;
          end;

        if iteminfoarray[i,3] <> 'M' then
          scorearray[row,3]:= scorearray[row,3] - convertresponsetointeger(FCh,iteminfoarray,i,itemscalearray[i,1]);
      ...
      FItemStatsArray[i,5]:= ((FNumberOfItems - 1) / (FNumberOfItems - 2)) * (1-(FPSum / (FVarianceForKR20Without + 0.000000000001)));    //need updated variance
    end; //if 'y'
end;

procedure TItemanCalculation.DomainStats(iteminfoarray, responsearray : TChar2Array; itemscalearray : TIntegerArray; domscalearray : TRealArray; nkeyarray : TIntegerArray);
var
...
begin
  ...
  begin
      ...
      for Row:= 1 to FNumberOfExaminees do
        ...
        begin
          ...
          begin
            ...
            if (iteminfoarray[lColumn,3] <> 'M') and (responsearray[row,lColumn] <> FNAChar) and (responsearray[row,lColumn] <> FOmitChar) then
              FDomainArray[row,domain] := FDomainArray[row,domain] + convertresponsetointeger(responsearray[row,lColumn],iteminfoarray,lColumn,itemscalearray[lColumn,1]);
          end;
        end;
    ...
    begin
      ...
      for Row:= 1 to FNumberOfExaminees do
      begin
        if FDomainArray[Row,Domain] < i then i:= FDomainArray[Row,Domain];
        if FDomainArray[Row,Domain] > j then j:= FDomainArray[Row,Domain];

        if (frmMain.standscale.IsChecked) and (FDomainStatsArray[domain,3] > 0) then
        begin
          rescaled:= FDomainArray[row,domain] * (StrToFloatDef(frmMain.newsdbox.Text, 0) / FDomainStatsArray[domain,3]);
          rescaled:= rescaled + (StrToFloatDef(frmMain.newmeanbox.Text, 0) - (StrToFloatDef(frmMain.newsdbox.Text, 0) / FDomainStatsArray[domain,3]) * FDomainStatsArray[domain,2]);
        end;

        if frmMain.linearscale.IsChecked then
          rescaled:= FDomainArray[row,domain]* StrToFloatDef(frmMain.slopebox.Text, 0) + StrToFloatDef(frmMain.interceptbox.Text, 0);

        domscalearray[row,domain]:= rescaled;
        lDomainSum:= lDomainSum + rescaled;

        if rescaled < mi then mi:= rescaled;
        if rescaled > mj then mj:= rescaled;
      ...
      for row:= 1 to FNumberOfExaminees do
        lDomainSS:= lDomainSS + (FDomScaleStat[domain,1] - domscalearray[row,domain])*(FDomScaleStat[domain,1] - domscalearray[row,domain]);
      ...
      begin
        ...
        for row:= 1 to 16 do dommean[row]:= 0; //reset to 0
        for row:= 1 to 16 do domcount[row]:= 0; //reset to 0
        ...
        begin
          ...
          for row:= 1 to FNumberOfExaminees do
            ...
            begin
              ...
              if (frmMain.spurbox.IsChecked = false) then FResponseAsInt := 0;

              TSD:= TSD + ((FDomainArray[row,domain]-FResponseAsInt) - TMean)*((FDomainArray[row,domain]-FResponseAsInt) - TMean);
            end;
          ...
          begin
            ...
            begin
              ...
              if frmMain.spurbox.IsChecked then
                temp4 := temp4 + (FDomainArray[row,domain] - itemscore - TMean) * (FDomainArray[row,domain] - itemscore - TMean);

              dommean[FResponseAsInt+Fwts]:= dommean[FResponseAsInt+Fwts] + (FDomainArray[row,domain] - itemscore);
              domcount[FResponseAsInt+Fwts]:= domcount[FResponseAsInt+Fwts] + 1;
              temp1:= temp1 + (FResponseAsInt - Fitemstatsarray[lColumn,2]) * (FResponseAsInt - Fitemstatsarray[lColumn,2]);
              temp2:= temp2 + (FResponseAsInt - Fitemstatsarray[lColumn,2]) * (FDomainArray[row,domain] - itemscore - TMean);
            end;  //if response is not omit FNAChar
          end;  //FNumberOfExaminees loop
          ...
          Fitemstatsarray[lColumn,7] := temp2/(temp3+1E-22); //domain R correlation for polytomous items
            ...
            for row:= 1 to FNumberOfExaminees do
              if responsearray[row,lColumn] <> FNAChar then
                ...
                begin
                  ...
                  if FCh = iteminfoarray[lColumn,1] then correct := true;
                  if (nkeyarray[lColumn,1] > 1) then
                    for k:= 1 to nkeyarray[lColumn,1] - 1 do
                      if upcase(iteminfoarray[lColumn,k+3]) = upcase(FCh) then
                        correct:= true;

                  if correct = true then resp := 1 else resp := 0;
                  if (resp = 1) and frmMain.spurbox.IsChecked then temp4:= 1 else temp4:= 0;
                  if (resp = 1) and frmMain.spurbox.IsChecked then temp2:= FP else temp2:= 0;
                  if (FP <> 0) and (FP <> 1) and (FDomainStatsArray[domain,3] <> 0) then
                    temp1:= temp1 + (((FDomainArray[row,domain]-temp4) - (FDomainStatsArray[domain,2]-temp2)) / FDomainStatsArray[domain,3]) * (resp - FP)/sqrt(FP*(1-FP)+1E-20);
                  ...
                  if Fitemstatsarray[lColumn,8] > 1.0 then Fitemstatsarray[lColumn,8] := 1.0;      {cap}
                  if Fitemstatsarray[lColumn,8] < -1.0 then Fitemstatsarray[lColumn,8] := -1.0;    {cap}
    ...
    for Domain1:= 1 to FNumDomains do
      ...
      begin
        ...
        for row:= 1 to FNumberOfExaminees do
          FDomainCS:= FDomainCS + (FDomainArray[Row, Domain1] - DomainMean1) * (FDomainArray[Row, Domain2] - DomainMean2);
...
procedure TItemanCalculation.DrawCsemGraph(nit: integer; graph: TBitmap;
  title: string; csemarray: TRealArray);
var
  MaxX, mvy,  incrY : real;
    ...
    begin
      ...
      if alttext = True then
        if nit >= 20 then
          ...
          FillText(Rect(xst - xadj + row * xsp, yst + 8, xst - xadj + row * xsp + 50, yst + 8 + 50),
            IntToStr(incr), False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
//      canvas.pen.Color := RGB(22, 22, 22);
    ...
    begin
      ...
      begin
        DrawLine(Point(GraphX, GraphY), Point(xst + round(row * MaxX / nit), yst - round(sqrt(csemarray[row, 2]) * GraphScaler)), 1);
...
procedure TItemanCalculation.DrawDistGraph(Min, Max: Real;
  nit, nunit, call: Integer; graph: TBitmap; title: string);
var
  // b1 : tpicture;
  MaxY, mvy, spacer, bar1: Real;
  i, j, ht, GraphScaler, GraphX, GraphY, xst, yst, MaxFr, incr, xadj, aj, xsp,
    row: Integer;
  alttext : boolean;
begin
  // Set size
...
//    DrawLine(Point(50, 250), Point(360, 250), 1); { X axis }
    ...
    begin
      ...
      if (Min + ((Max - Min) / 10) * row) < 9.5 then
        xadj := 2; // adjusting the position of the xaxis labels
      if ((Min + ((Max - Min) / 10) * row) >= 99.5) and
        ((Min + ((Max - Min) / 10) * row) < 999.5) then
        xadj := 10;
      if ((Min + ((Max - Min) / 10) * row) >= 999.5) and
        ((Min + ((Max - Min) / 10) * row) < 9999.5) then
        xadj := 14;
      ...
      if (alttext = True) and (call = 1) then
        if brange >= 20 then
          FillText(Rect(xst - xadj + row * xsp, yst + 8, xst - xadj + row * xsp + 50, yst + 8 + 50),
            formatfloat('0', (Min + ((Max - Min) / (aj + 1E-20)) * row)), False, 1, [], TTextAlign.Leading, TTextAlign.Leading)
        else
          FillText(Rect(xst - xadj + row * xsp, yst + 8, xst - xadj + row * xsp + 50, yst + 8 + 50),
...
//      pen.Color := RGB(22, 22, 22);
    ...
    if call = 3 then
      for j := 0 to nbin do
        FillText(Rect(xst - 6 + j * xsp, yst + 8, xst - 6 + j * xsp + 50, yst + 8 + 50),
          formatfloat('0.0', Min + (brange / (aj + 1E-20)) * j), False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
    if call = 4 then
      for j := 0 to nbin do
      begin
        if Min + 0.1 * j < 0 then
    ...
    begin
      ...
      GraphX := GraphX + xsp; // xsp needs to be adjusted if nbin equals 20 or 18
...
procedure TItemanCalculation.DrawItemGraph(graph: TBitmap; item: integer;
  ItemScaleArray: TIntegerArray; ItemIDArray: TStringArray; key1, start0: char);
var
...
//    DrawBitmap(frmMain.refill.Bitmap, RectF(0, 0, 450, 300), RectF(0, 0, 450, 300), 1);
    ...
    begin
      ...
      for i := 1 to mf - 1 do
        if (storage[i] - storage[i + 1] < 0.07) and
          (ltr[altno[i]] = ltr[altno[i + 1]]) then
          ltr[altno[i + 1]] := ltr[altno[i + 1]] + 9;
      for i := 1 to mf - 2 do
        if (storage[i] - storage[i + 2] < 0.07) and
          (ltr[altno[i]] = ltr[altno[i + 2]]) then
          ltr[altno[i + 2]] := ltr[altno[i + 2]] + 9;
      for i := 1 to mf - 3 do
        if mf > 3 then
          if (storage[i] - storage[i + 3] < 0.07) and
            (ltr[altno[i]] = ltr[altno[i + 3]]) then
            ltr[altno[i + 3]] := ltr[altno[i + 3]] + 9;
      for i := 1 to mf - 4 do
        if mf > 4 then
          if (storage[i] - storage[i + 4] < 0.07) and
            (ltr[altno[i]] = ltr[altno[i + 4]]) then
            ltr[altno[i + 4]] := ltr[altno[i + 4]] + 9;
      for i := 1 to mf - 5 do
        if mf > 5 then
          if (storage[i] - storage[i + 5] < 0.07) and
            (ltr[altno[i]] = ltr[altno[i + 5]]) then
            ltr[altno[i + 5]] := ltr[altno[i + 5]] + 9;
      for i := 1 to mf - 6 do
        if mf > 6 then
          if (storage[i] - storage[i + 6] < 0.07) and
            (ltr[altno[i]] = ltr[altno[i + 6]]) then
            ltr[altno[i + 6]] := ltr[altno[i + 6]] + 9;
      for i := 1 to mf - 7 do
        if mf > 7 then
          if (storage[i] - storage[i + 7] < 0.07) and
            (ltr[altno[i]] = ltr[altno[i + 7]]) then
            ltr[altno[i + 7]] := ltr[altno[i + 7]] + 9;
      for i := 1 to mf - 8 do
        if mf > 8 then
          if (storage[i] - storage[i + 8] < 0.07) and
            (ltr[altno[i]] = ltr[altno[i + 8]]) then
    ...
    begin
      ...
      begin
        ...
        DrawEllipse(Rect((j - 1) * xspan + 98, 250 - tmpY - 2,
          (j - 1) * xspan + 102, 250 - tmpY + 2), 1);
      ...
      begin
        if z = 1 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '0', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 2 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '1', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 3 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '2', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 4 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '3', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 5 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '4', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 6 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '5', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 7 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '6', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 8 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '7', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 9 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '8', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
      end;
      if (Frespasint = True) and (start0 <> 'P') then
      begin
        if z = 1 then
        ...
        if z = 9 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            '9', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
      end;
      if (Frespasint = False) and (start0 <> 'P') then
      begin
        if z = 1 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'A', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 2 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'B', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 3 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'C', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 4 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'D', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 5 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'E', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 6 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'F', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 7 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'G', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 8 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'H', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
        if z = 9 then
          FillText(Rect(314 + ltr[i], 242 - GraphY, 314 + ltr[i] + 50, 242 - GraphY + 50),
            'I', False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
      end;
    end;
    ...
    GraphX := 100;
    DrawLine(Point(GraphX, 245), Point(100, 255), 1); { Hash marks... }

    for j := 2 to Fnumcut do
    begin
      GraphX := 100 + (j - 1) * xspan;
      DrawLine(Point(GraphX, 245), Point(GraphX, 255), 1); { Hash marks... }
    end;
    ...
    for j := 1 to Fnumcut do
      FillText(Rect(95 + (j - 1) * xspan, 260, 95 + (j - 1) * xspan + 50, 310),
        IntToStr(j), False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
    Font.Size := 14;
    ...
    if IntToStr(item) = itemidarray[item] then
      ...
      mf := 0;
    FillText(Rect(150 + mf, 10, 250 + mf, 60), 'Item ' + IntToStr(item) + colo +
      ideq, False, 1, [], TTextAlign.Leading, TTextAlign.Leading); { Title }
...
procedure TItemanCalculation.DrawScatterplot(Min, Max: Real;
  nit, nunit, call: Integer; graph: TBitmap; iteminfoarray: TChar2Array;
...
//      pen.Color := RGB(22, 22, 22);
    ...
    begin
      ...
      if (BndR >= 1.3) and (j mod 2 <> 0) then
        FillText(Rect(30 + ii, round(yst - (j * 0.1) * GraphScaler) - 8, 30 + ii + 50, round(yst - (j * 0.1) * GraphScaler) - 8 + 50),
          formatfloat('0.0', (j * 0.1) - (BndR - 1)), False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
      if BndR < 1.3 then
        FillText(Rect(30 + ii, round(yst - (j * 0.1) * GraphScaler) - 8, 30 + ii + 50, round(yst - (j * 0.1) * GraphScaler) - 8 + 50),
          formatfloat('0.0', (j * 0.1) - (BndR - 1)), False, 1, [], TTextAlign.Leading, TTextAlign.Leading);
...
function TItemanCalculation.EstimateKR20(Domain: Integer;
  iteminfoarray: TChar2Array; itemscalearray: TIntegerArray): Real;
var
  lRow: Integer;
begin
  FPSum := 0.0;
  ...
  for lRow := 1 to FNumberOfItems do
    ...
    begin
      ...
      FPSum:= FPSum + FItemStatsArray[lRow,6];  //FItemStatsArray[lRow,6] has item variance
    end;

  if FDomainStatsArray[Domain,1] > 1 then
    Result:= (FDomainStatsArray[Domain, 1] / (FDomainStatsArray[Domain, 1] - 1)) * (1 - (FPSum / ((FDomainStatsArray[Domain, 3] * FDomainStatsArray[Domain, 3]) + 0.000000000001)))
  else
    Result:= 0;
end;

function TItemanCalculation.CalculateMeanP(Domain : integer; ItemInfoArray : TChar2Array; ItemScaleArray : TIntegerArray): Real;
var
  lRow: Integer;
begin
  FPSum:= 0.0;
...
procedure TItemanCalculation.CalcGF(Min, Max: real; call: integer;
  scorearray: TIntegerArray);
var
  binc, row, counter, rawsc : integer;
  TableInterval : real;
{Calculate Grouped Frequency Table}
begin
    for counter := 1 to nbin do begin
     for row := 1 to 4 do GroupedFreqArray[counter,row] := 0;
    end; //clearing the array

    binc := nbin-1;
    if (max-min < 20) and (max < 20) then binc := binc - 1;
    if (max-min < 20) and (max < 20) then TableInterval := 1
      else tableinterval := (max-min)/binc;
    GroupedFreqArray[1,1] := Min-0.00000000001;
    GroupedFreqArray[nbin,2] := Max+0.0000000001;
    GroupedFreqArray[1,3] := 0;
      For counter := 2 to nbin do begin
     ...
     For counter := 1 to FNumberofexaminees do begin
       For row := 1 to nbin do begin
         if call = 1 then rawsc := scorearray[counter,1] else rawsc := Fdomainarray[counter,call-1];
         If rawsc >= GroupedFreqArray[row,1] then begin
          If rawsc < GroupedFreqArray[row,2] then begin
...
procedure TItemanCalculation.CalcItemGF(Min, Max: real; call, denom: integer;
  type1: char; iteminfoarray: TChar2Array);
var row, counter : integer;
...
begin
    ...
    if (type1 <> 'M') and (max-min < 1) then tableinterval := 0.1 else TableInterval := (Max-Min)/(nbin+1E-20);
    if (maxcategory > 2) and (call <> 4) then begin
    if (type1 <> 'M') and (max-min < 1) then spacer := (1 - (max-min)) /2;
    if (type1 <> 'M') and (max-min < 1) then min := min - spacer;
    if (type1 <> 'M') and (max-min < 1) then max := max + spacer;
    end;
    GroupedFreqArray[1,1] := Min-0.00000000001;
    GroupedFreqArray[nbin,2] := Max+0.0000000001;
    GroupedFreqArray[1,3] := 0;
     ...
     For counter := 1 to Fnumberofitems do begin
      if (iteminfoarray[counter,3] = 'P') and (type1 = 'R') then type1 := 'P';
      if (iteminfoarray[counter,3] = 'R') and (type1 = 'P') then type1 := 'R';
       if (iteminfoarray[counter,3] = type1) and (iteminfoarray[counter,2] = 'Y') then begin   //type1 is set to be M or R
        For row := 1 to nbin do begin
         if call = 1 then rawsc := Fitemstatsarray[counter,2] else rawsc := Fitemstatsarray[counter,3];
           //writeln(outputfile, rawsc:20:23, ',', GroupedFreqArray[row,1]:20:23, ',', GroupedFreqArray[row,2]:20:23, ',', GroupedFreqArray[row,3]:20:23);     //QA
          If rawsc-GroupedFreqArray[row,2] < 0.0000000001 then rawsc := rawsc + 0.0000000001;    //this helps when 0.7 is stored as 0.699999999999999 as bytes
          if rawsc > 1.0 then rawsc := 1.0;
         If rawsc >= GroupedFreqArray[row,1] then begin
           If rawsc < GroupedFreqArray[row,2] then begin
           GroupedFreqArray[row,3] := GroupedFreqArray[row,3] + 1;
           end;
...
function TItemanCalculation.CalculateItemMean(Domain : integer; ItemInfoArray : TChar2Array; ItemScaleArray : TIntegerArray): Real;
var
  lRow: Integer;
begin
  FPSum:= 0.0;
...
function TItemanCalculation.CalculateMeanRpbis(Domain : integer; ItemInfoArray : TChar2Array; ItemScaleArray : TIntegerArray): Real;
var
  lRow: Integer;
begin
  FPSum:= 0.0;
...
procedure TItemanCalculation.Reliability(splitscore1, splitscore2, splitdomain1, splitdomain2, itemscalearray : TIntegerArray; iteminfoarray, responsearray : TChar2Array);
var
...
begin
  ...
  for column:= 1 to FNumberOfItems do
    ...
    begin
      ...
      if i >= num1 then rand := 0; //force item into FGroup 2 if half in FGroup 1
      if j >= num1 then rand := 1; //force item into FGroup 1 if half in FGroup 2
      if rand >= 0.50 then i := i + 1;
      if rand < 0.50 then j := j + 1;
      ...
      for row:= 1 to FNumberOfExaminees do
        if (upcase(responsearray[row,column]) <> upcase(FNAchar)) and (upcase(responsearray[row,column]) <> upcase(FOmitChar)) then  //omits and nas are excluded
        begin
          FCh:= responsearray[row,column];
          ...
          begin
            ...
            if iteminfoarray[column,3] <> 'M' then
              splitscore1[row,1]:= splitscore1[row,1] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
          end;

          if rand < 0.50 then
          begin
            if iteminfoarray[column,3] = 'M' then if upcase(iteminfoarray[column,1]) = upcase(responsearray[row,column]) then
              splitscore2[row,1] := splitscore2[row,1] + 1;

            if iteminfoarray[column,3] <> 'M' then
              splitscore2[row,1] := splitscore2[row,1] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
          end;
          ...
          begin
            ...
            if iteminfoarray[column,3] <> 'M' then
              splitscore1[row,3] := splitscore1[row,3] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
          end;
          ...
          begin
            ...
            if iteminfoarray[column,3] <> 'M' then
              splitscore2[row,3] := splitscore2[row,3] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
          end;
          ...
          begin
            ...
            if iteminfoarray[column,3] <> 'M' then
              splitscore1[row,5] := splitscore1[row,5] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
          end;
          ...
          begin
            ...
            if iteminfoarray[column,3] <> 'M' then
              splitscore2[row,5] := splitscore2[row,5] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
          end;
  ...
  begin
    ...
    for column:= 1 to FNumberOfItems do
      ...
      begin
        ...
        if i >= num2 then rand:= 0; //force item into FGroup 2 if half in FGroup 1
        if j >= num2 then rand:= 1; //force item into FGroup 1 if half in FGroup 2
        if rand >= 0.50 then i:= i + 1;
        if rand < 0.50 then j:= j + 1;
        ...
        for row:= 1 to FNumberOfExaminees do
          if (upcase(responsearray[row,column]) <> upcase(FNAchar)) and (upcase(responsearray[row,column]) <> upcase(FOmitChar)) then
          begin  //omits and nas are excluded
            ...
            begin
              ...
              if iteminfoarray[column,3] <> 'M' then
                splitscore1[row,2] := splitscore1[row,2] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
            end;
            ...
            begin
              ...
              if iteminfoarray[column,3] <> 'M' then
                splitscore2[row,2] := splitscore2[row,2] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
            end;
            ...
            begin
              ...
              if iteminfoarray[column,3] <> 'M' then
                splitscore1[row,4] := splitscore1[row,4] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
            end;
            ...
            begin
              ...
              if iteminfoarray[column,3] <> 'M' then
                splitscore2[row,4] := splitscore2[row,4] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
            end;
            ...
            begin
              ...
              if iteminfoarray[column,3] <> 'M' then
                splitscore1[row,6] := splitscore1[row,6] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
            end;
            ...
            begin
              ...
              if iteminfoarray[column,3] <> 'M' then
                splitscore2[row,6] := splitscore2[row,6] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
            end;
          end; //for row loop
      end; //for column loop -- items loop
    ...
    begin
      ...
      for column:= 1 to FNumberOfItems do
        ...
        begin
          ...
          if i >= domnum[lDomain] then rand := 0; //force item into FGroup 2 if half in FGroup 1
          if j >= domnum[lDomain] then rand := 1; //force item into FGroup 1 if half in FGroup 2
          if rand >= 0.50 then i := i + 1;
          if rand < 0.50 then j := j + 1;
          ...
          for row := 1 to FNumberOfExaminees do
            ...
            begin  //omits and nas are excluded
              ...
              begin
                ...
                if iteminfoarray[column,3] <> 'M' then
                  splitdomain1[row,1+cnum] := splitdomain1[row,1+cnum] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
              end;
              ...
              begin
                ...
                if iteminfoarray[column,3] <> 'M' then
                  splitdomain2[row,1+cnum] := splitdomain2[row,1+cnum] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
              end;
              ...
              begin
                ...
                if iteminfoarray[column,3] <> 'M' then
                  splitdomain1[row,2+cnum] := splitdomain1[row,2+cnum] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
              end;
              ...
              begin
                ...
                if iteminfoarray[column,3] <> 'M' then
                  splitdomain2[row,2+cnum] := splitdomain2[row,2+cnum] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
              end;
              ...
              begin
                ...
                if iteminfoarray[column,3] <> 'M' then
                  splitdomain1[row,3+cnum] := splitdomain1[row,3+cnum] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
              end;
              ...
              begin
                ...
                if iteminfoarray[column,3] <> 'M' then
                  splitdomain2[row,3+cnum] := splitdomain2[row,3+cnum] + ConvertResponseToInteger(FCh,iteminfoarray,column,itemscalearray[column,1]);
              end;
            end; //for row loop
        end; //for column loop -- items loop
  ...
  begin
    ...
    if FPreTest > 0 then
    begin
      temp[3] := temp[3] + splitscore1[row,2];     //split half - scored items only
      temp[4] := temp[4] + splitscore2[row,2];
      temp[7] := temp[7] + splitscore1[row,4];     //first-last - scored items only
      temp[8] := temp[8] + splitscore2[row,4];
      temp[11] := temp[11] + splitscore1[row,6];     //odd-even - scored items only
  ...
  begin
    ...
    if FNumDomains > 1 then
      ...
      begin
        cnum := 3*(lDomain-1);
        domSq[1,lDomain] := domSq[1,lDomain] + (splitdomain1[row,1+cnum] - domM[1,lDomain])*(splitdomain1[row,1+cnum] - domM[1,lDomain]);
        domSq[2,lDomain] := domSq[2,lDomain] + (splitdomain2[row,1+cnum] - domM[2,lDomain])*(splitdomain2[row,1+cnum] - domM[2,lDomain]);
        domSq[3,lDomain] := domSq[3,lDomain] + (splitdomain1[row,2+cnum] - domM[3,lDomain])*(splitdomain1[row,2+cnum] - domM[3,lDomain]);
        domSq[4,lDomain] := domSq[4,lDomain] + (splitdomain2[row,2+cnum] - domM[4,lDomain])*(splitdomain2[row,2+cnum] - domM[4,lDomain]);
        domSq[5,lDomain] := domSq[5,lDomain] + (splitdomain1[row,3+cnum] - domM[5,lDomain])*(splitdomain1[row,3+cnum] - domM[5,lDomain]);
        domSq[6,lDomain] := domSq[6,lDomain] + (splitdomain2[row,3+cnum] - domM[6,lDomain])*(splitdomain2[row,3+cnum] - domM[6,lDomain]);
      end;
    ...
    if FNumDomains > 1 then
      ...
      begin
        cnum := 3*(lDomain-1);
        domcp[1,lDomain] := domcp[1,lDomain] + (splitdomain1[row,1+cnum] - domM[1,lDomain])*(splitdomain2[row,1+cnum] - domM[2,lDomain]);
        domcp[2,lDomain] := domcp[2,lDomain] + (splitdomain1[row,2+cnum] - domM[3,lDomain])*(splitdomain2[row,2+cnum] - domM[4,lDomain]);
        domcp[3,lDomain] := domcp[3,lDomain] + (splitdomain1[row,3+cnum] - domM[5,lDomain])*(splitdomain2[row,3+cnum] - domM[6,lDomain]);
      end;
  ...
  if FNumDomains > 1 then
    ...
    begin
      ...
      Fdomainstatsarray[lDomain,10]:= domcp[1,lDomain]/(sd1*sd2*(FNumberOfExaminees-1)+1E-20); //split half correlation
      Fdomainstatsarray[lDomain,13] := 2*Fdomainstatsarray[lDomain,10] / (1 + Fdomainstatsarray[lDomain,10]+1E-20); //spearman brown
      sd1 := sqrt(domsq[3,lDomain]/(FNumberOfExaminees-1+1E-20));
      sd2 := sqrt(domsq[4,lDomain]/(FNumberOfExaminees-1+1E-20));
      Fdomainstatsarray[lDomain,11]:= domcp[2,lDomain]/(sd1*sd2*(FNumberOfExaminees-1)+1E-20); //split half correlation
      Fdomainstatsarray[lDomain,14] := 2*Fdomainstatsarray[lDomain,11] / (1 + Fdomainstatsarray[lDomain,11]+1E-20); //spearman brown
      sd1 := sqrt(domsq[5,lDomain]/(FNumberOfExaminees-1+1E-20));
      sd2 := sqrt(domsq[6,lDomain]/(FNumberOfExaminees-1+1E-20));
      Fdomainstatsarray[lDomain,12]:= domcp[3,lDomain]/(sd1*sd2*(FNumberOfExaminees-1)+1E-20); //split half correlation
      Fdomainstatsarray[lDomain,15] := 2*Fdomainstatsarray[lDomain,12] / (1 + Fdomainstatsarray[lDomain,12]+1E-20); //spearman brown
    end;
end;

procedure TItemanCalculation.CSem(itemscalearray : TIntegerArray; iteminfoarray, responsearray : TChar2Array; csemarray : TRealArray);
var
...
begin
   ...
   for column:= 1 to FNumberOfItems do
     ...
     begin
       pss1:= pss1 + (Fitemstatsarray[column,2] - Fteststatsarray[1,7]) * (Fitemstatsarray[column,2] - Fteststatsarray[1,7]);
       multch:= multch + 1;
     end;
   ...
   denom:= Fteststatsarray[1, 2] * (multch - Fteststatsarray[1,2]) - Fteststatsarray[1, 3] * Fteststatsarray[1, 3] - multch * (pvar1);
   K:= num / (denom + 1E-20);
...
begin
  ...
  if FIX < 0.001 then FIX:= FIX + 30269;
  if FIY < 0.001 then FIY:= FIY + 30307;
  if FIZ < 0.001 then FIZ:= FIZ + 30323;
...
begin
  ...
  repeat
    ...
    begin
      ...
      Jumpout: numericarray[j,0]:= Value;
    end;
  until (h = 1);
end;
...
var
  N: integer;  //Same as H-H Errors In Common - need to sum before final calculation
  k: integer;  //Same as H-H Exact Errors In Common - need to sum before final calculation
    ...
    for exam2:= 1 to FNumberofExaminees do {Compare examinee 1 to N}
      ...
      begin
        for item:= 1 to FNumberofItems do
          if (iteminfoarray[item,2] = 'Y') and// exclude 'N' and 'P' items
             (ResponseArray[exam1,item] <> FOmitChar) and (ResponseArray[exam1,item] <> FNAChar) and
             (ResponseArray[exam2,item] <> FOmitChar) and (ResponseArray[exam2,item] <> FNAChar) // exclude omit and na items
          then
          begin
            r1:= ResponseArray[exam1,item];    {response from examinee 1 for item i}
            r2:= ResponseArray[exam2,item];    {response from examinee 2 for item i}
            ...
            FEICArray[exam1,exam2]:= (FEICArray[exam1,exam2]  + (w1 * w2)); // count 1 if both incorrect
            FEEICArray[exam1,exam2]:= (FEEICArray[exam1,exam2]  + (w1 * w3)); // count 1 if both exactly incorrect
          end;
        ...
        begin
          BB:= factorial(N)/(factorial(k_step) * factorial(N - k_step) + 0.0000000000000001) * power(lBBO_P, k_step) * power((1 - lBBO_P), (N-k_step));
...
begin
  Result:= 0;
  if iteminfoarray[item,1] = 'A' then Result:= 1;
  if iteminfoarray[item,1] = 'B' then Result:= 2;
  if iteminfoarray[item,1] = 'C' then Result:= 3;
  if iteminfoarray[item,1] = 'D' then Result:= 4;
  if iteminfoarray[item,1] = 'E' then Result:= 5;
  if iteminfoarray[item,1] = 'F' then Result:= 6;
  if iteminfoarray[item,1] = 'G' then Result:= 7;
  if iteminfoarray[item,1] = 'H' then Result:= 8;
  if iteminfoarray[item,1] = 'I' then Result:= 9;
  if iteminfoarray[item,1] = '0' then Result:= 0;
  if iteminfoarray[item,1] = '1' then Result:= 1;
  if iteminfoarray[item,1] = '2' then Result:= 2;
  if iteminfoarray[item,1] = '3' then Result:= 3;
  if iteminfoarray[item,1] = '4' then Result:= 4;
  if iteminfoarray[item,1] = '5' then Result:= 5;
  if iteminfoarray[item,1] = '6' then Result:= 6;
  if iteminfoarray[item,1] = '7' then Result:= 7;
  if iteminfoarray[item,1] = '8' then Result:= 8;
  if iteminfoarray[item,1] = '9' then Result:= 9;
end;
...
begin
  ...
  begin
    ...
    if scoredclass.IsChecked then
     ...
     begin
       if standscale.IsChecked then
       begin
         zscore := (classcutbox.Text.ToDouble - newmeanbox.Text.ToDouble) /
           newsdbox.Text.ToDouble;
         rawcut := (zscore * Fteststatsarray[1, 3]) + Fteststatsarray[1, 2];
         pval := rawcut / Fteststatsarray[1, 1];
       end;
       if linearscale.IsChecked then
       begin
         xbar := (Fteststatsarray[1, 2] + interceptbox.Text.ToDouble) *
           slopebox.Text.ToDouble;
 ...
 procedure TItemanCalculation.SubGroups(aItemInfoArray, aResponseArray
   : TChar2Array; aOmch: Char; aI: Integer; aRankIngArray, aReduceArray,
   aItemsCaleArray: TIntegerArray);
 var
 ...
 begin
    ...
    for lRow:= 1 to FNumberOfExaminees do
      if aResponseArray[lRow, aI] <> FOmitChar then
        ...
        begin
          ...
          FSubGroupPArray[((FResponseAsInt - 1 + Fwts) * FNumCut + FGroup)]:= FSubGroupPArray[((FResponseAsInt-1+Fwts) * FNumCut + FGroup)] + 1;
  ...
  begin
    ...
    begin
      for jfor:= 1 to FNumCut do
        FSubGroupPArray[lColumn + pred(jfor)]:= FSubGroupPArray[lColumn + pred(jfor)] / (FGroupCounts[jfor] - aReduceArray[aI, jfor] + 1E-20);
