...
uses
  System.Types, System.Classes, System.SysUtils, System.StrUtils, System.IOUtils,

  {$IF DEFINED(Win64) or DEFINED(Win32)}
  System.Win.Registry, WinAPI.ShellApi, Winapi.Windows,
  {$ENDIF}
...
begin
  case FLicenseStatus of
    lcsDemo: Result:= 'Demo License';
    lcsActive: Result:= 'Active License';
    lcsWillExpire: Result:= Format('The licence will expire in %d days', [FExpirationDays]);
    lcsExpired: Result:= 'License Expired';
  end;

  if (ASupInfo) then
    case FLicenseStatus of
      lcsDemo: Result:= Result + ' (click here to register)';
      lcsWillExpire, lcsExpired: Result:= Result + ' (click here to renew)';
    end;
end;
...
begin
  ...
  GetVolumeInformation(pWideChar(ExtractFileDir(lSysDriveName)), nil, 0, @lVolumeSerialNumber, lMaximumComponentLength, lFileSystemFlags, nil, 0);
  lSerialNumber:= IntToHex(HiWord(Integer(lVolumeSerialNumber)), 4) + IntToHex(LoWord(Integer(lVolumeSerialNumber)), 4);
  {$ENDIF}
...
begin
  ...
  try
    ...
    // calc a sum of the license
    iTot := 0;
    for iCount := 1 to Length(sStr) do begin
...
begin
  ...
  begin
    ...
    // 2nd = 2..3     5th = 6..7
    sDate:= sDate[1] + Char(StrToInt(sDate[2] + sDate[3])) + sDate[4] + sDate[5] + Char(StrToInt(sDate[6] + sDate[7])) + Copy(sDate, 8, Length(sDate));
...
begin
  ...
  begin
    sExp:= ReverseString(IntToStr(Trunc(Int(AValidUntil)))); // date in string without the decimal part, reversed
    sExp:= RandomFrom(RAND_VAL) + Copy(sExp, 1, 2) + RandomFrom(RAND_VAL) + Copy(sExp, 3, Length(sExp)); // i add a random number at pos 1 and 3
    sExp:= sExp[1] + IntToStr(Ord(sExp[2])) + sExp[3] + sExp[4] + IntToStr(Ord(sExp[5])) + Copy(sExp, 6, Length(sExp)); // the second and 5th characters are recalculated as the ord of the char
