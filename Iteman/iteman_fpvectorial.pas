...
Uses
  ...
  fpimage, fpcanvas, FPWriteJPEG,
  fpvectorial, fpvutils, lazutf8, Math;

Type
  TvTextPageSequenceHelper = class helper for TvTextPageSequence
  class var
    vTopMargin: double;
    vBottomMargin: double;
    vLeftMargin: double;
    vRigthMargin: double;
  end;
  ...
  TFileType = (ftContentTypes, ftRelationships, ftDocRelationships, ftMedia);
  ...
  Public
    ...
    Image : TvRasterImage;  { TODO: How are we going to handle images? }
  ...
  { TvDOCXVectorialWriterEx }


  TvDOCXVectorialWriterEx = Class(TvCustomVectorialWriter)
  Private
...
Const
  ...
  OOXML_TYPE_MEDIA = OOXML_RELS + '/image';
  ...
  OOXML_CONTENTTYPE_JPEG = OOXML_CONTENTTYPE + '.numbering+xml';

  // Shared between document.xml and each header.xml/footer.xml
  OOXML_DOCUMENT_NAMESPACE =
    sLineBreak+' xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"'+
    sLineBreak+' xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"'+
    sLineBreak+' xmlns:v="urn:schemas-microsoft-com:vml"'+sLineBreak;
...
Begin
  ...
  If i > 0 Then
    Result := PrefixFileName + Copy(Path, i + 1, Length(Path) - i)
  Else
    Result :=  PrefixFileName + Path;
End;
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      try
        FStream := TMemoryStream.Create;
        Image.RasterImage.SaveToStream(FStream, W);
        FStream.Position := 0;
      finally
        w.Free;
      end;
...
Constructor TvDOCXVectorialWriterEx.Create;
Begin
  Inherited Create;
...
Destructor TvDOCXVectorialWriterEx.Destroy;
Begin
...
Function TvDOCXVectorialWriterEx.PrepareContentTypes: String;
Var
  i: Integer;
Begin
  Result := XML_HEADER + LineEnding +
    ...
    LineEnding +
    '  <Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>'+ LineEnding + '  <Default Extension="xml" ContentType="application/xml"/>' +  LineEnding+
    '  <Default Extension="jpg" ContentType="image/jpeg"/>' +  LineEnding+
    '  <Default Extension="png" ContentType="image/png"/>' +  LineEnding;
...
Function TvDOCXVectorialWriterEx.PrepareRelationships: String;
Var
  i: Integer;
Begin
  Result := XML_HEADER + LineEnding +
...
Function TvDOCXVectorialWriterEx.PrepareDocRelationships: String;
Var
  i: Integer;
Begin
  Result := XML_HEADER + LineEnding +
...
Procedure TvDOCXVectorialWriterEx.PrepareDocument;
Var
    ...
    Begin
      ...
      Begin
        ...
        Begin
          ...
          //If i > iStart Then   // < if Length(sText) = 1 ?
          //Begin
            ...
            if sTemp = '<#Page#>' then
              oDocXML.Add('  <w:br w:type="page"/>') else //< page break 06.02.2017 19:20:44
  ...
  Procedure ProcessImage(AImage : TvRasterImage);
  var
    f: TFileInformation;
    s: string;
    w, h: Integer;
  begin
    w := AImage.RasterImage.Width;
    h := AImage.RasterImage.Height;
    s := OOXML_PATH_MEDIA+ '/image'+IntToStr(FFiles.Count)+'.jpg';
    f := FFiles.AddXMLFile('', s, OOXML_TYPE_MEDIA, [ftMedia, ftDocRelationships]);
    f.Image := AImage;
    f.XML.Free;
    f.XML := nil;
    f.PrefixFileName:= 'media/';
    s := Format('<w:pict>'+sLineBreak+
                '<v:shape id="pic%s" style="width:%d; height:%d">'+sLineBreak+
                '<v:imagedata r:id="%s"/>'+sLineBreak+
                '</v:shape>'+sLineBreak+
                '</w:pict>',
      [f.ID, w, h, f.ID]);
    oDocXML.Add(s);
  ...
  Begin
    ...
    Begin
      ...
      else if oEntity is TvRasterImage then
        ProcessImage(TvRasterImage(oEntity))
      Else
  ...
  const
    C_MM_ToMV = 56.6929;//13385826771653543307086614;
  Var
  ...
  Begin
    ...
    sTemp := Format('<w:pgMar w:top="%d" w:right="%d" w:bottom="%d" w:left="%d" w:header="0" w:footer="0" w:gutter="0"/>',
      [Round(APageSequence.vTopMargin * C_MM_ToMV),
      Round(APageSequence.vRigthMargin * C_MM_ToMV),
      Round(APageSequence.vBottomMargin * C_MM_ToMV),
      Round(APageSequence.vLeftMargin * C_MM_ToMV)]);
    oDocXML.Add(sTemp);
...
Function TvDOCXVectorialWriterEx.StyleNameToStyleID(AStyle: TvStyle): String;
Begin
...
Procedure TvDOCXVectorialWriterEx.PrepareTextRunStyle(ADoc: TIndentedStringList;
  AStyle: TvStyle);
Begin
...
Procedure TvDOCXVectorialWriterEx.PrepareStyles;
Var
  sType, sTemp: String;
  i: Integer;
...
Procedure TvDOCXVectorialWriterEx.PrepareNumbering;
Var
  oXML: TIndentedStringList;
  oStyle: TvListStyle;
  oFile: TFileInformation;
...
Procedure TvDOCXVectorialWriterEx.WriteToFile(AFileName: String;
...
Procedure TvDOCXVectorialWriterEx.WriteToStream(AStream: TStream;
  AData: TvVectorialDocument);
Var
...
//  RegisterVectorialWriter(TvDOCXVectorialWriterEx, vfDOCX);
end.
